...
func evalSLA(rows []Result) []slaVerdict {
	...
	out := []slaVerdict{
		...
		// Sheet says "p99 > 40 tok/s". Speed is higher-is-better, so the
		// customer-relevant tail is the slow end (p1), reported alongside p99.
...
func gateMs(name string, want int, samples []int, p float64) slaVerdict {
	if len(samples) == 0 {
		return slaVerdict{Name: name, Want: fmt.Sprintf("<%dms", want), Got: "n/a", Skip: true}
	}
...
func gateTPOT(name string, want float64, samples []float64, p float64) slaVerdict {
	if len(samples) == 0 {
		return slaVerdict{Name: name, Want: fmt.Sprintf(">%.0f tok/s", want), Got: "n/a", Skip: true}
	}
