...
func buildPayload(cls payloadClass, model string, seq int, maxTokens int, temperature *float64, toolsMode string, cachePrefix string) (builtPayload, error) {
	msgs := []chatMessage{
		{Role: "system", Content: "你是一个谨慎的编程助手。根据仓库上下文回答，不要编造文件内容。用简体中文回复。"},
	}
	if cls.Stream && cls.CacheTokens > 0 {
		prefix := cachePrefix
		if prefix == "" {
