...
推荐优先使用 CC-Switch 生成和管理这两个文件；不想安装工具时，再手动编辑。

##### 方式 A：CC-Switch 一键配置 Codex（最推荐）

如果你在 [3.2 CLI 版本：安装与配置](#32-cli-版本安装与配置) 中已安装 CC-Switch，可以直接用它配置 Codex：
...
- 标注要求：
  ...
  2. 用红色箭头指向 "+" 或 "Add Provider" 按钮
  3. 在表单中用红色方框框住 API Base URL，添加文字标注："注意末尾有 /v1"
  4. 用红色方框框住 API Key 输入框
  5. 用红色箭头指向 "Enable" / "Switch" 按钮
-->

CC-Switch 会自动生成 `config.toml` 和 `auth.json`，无需手动编辑。配置完成后，继续看本小节的第三步验证连接。
...
> 以下是 Desktop 桌面版的完整流程：安装 → 使用 CC-Switch 配置 Codex → 重启 Desktop → 验证连接。
...
#### 第二步：先用 CC-Switch 配置 ZeroCode Provider

Codex Desktop 当前不建议在应用里找 ZeroCode 的 Base URL 配置入口。正确做法是先用 **CC-Switch** 写入 Codex 的本地配置文件：`.codex/config.toml` 和 `.codex/auth.json`。

配置方法与 CLI 完全相同，请直接按 [4.2 CLI 版本：安装与配置](#42-cli-版本安装与配置) 中的「第二步 → 方式 A：CC-Switch 一键配置 Codex」操作：

1. 打开 CC-Switch，在左侧找到 **Codex**（或 **Codex CLI**）
2. 添加 **ZeroCode** Provider
3. 填写 `https://zerocode.kaynlab.com/v1` 和你的 API Key
4. 保存并切换到 **ZeroCode**

> 这里配置一次即可。Codex Desktop 和 Codex CLI 读取同一套 `.codex` 配置，因此不需要为 Desktop 单独维护第二份 API 配置。
...
- 标注要求：
  1. 用红色方框框住本地项目选择区域
  2. 用红色方框框住 Local 模式
  3. 添加文字标注："配置 CC-Switch 后重启 Desktop，再进入本地项目测试"
-->
...
| 错误现象 | 可能原因 | 解决方法 |
|---------|---------|---------|
| Desktop 仍然像官方账号一样请求 | CC-Switch Provider 未启用或 Desktop 未重启 | 回到 CC-Switch 确认 ZeroCode 已启用，然后完全退出并重启 Desktop |
| 提示认证失败 | API Key 未写入或 auth.json 不正确 | 用 CC-Switch 重新保存 Provider，或检查 `.codex/auth.json` |
