...
**平台地址：** 请使用管理员提供的实际平台域名（下文中所有 `https://你的平台域名` 均需替换为实际地址）
...
- 标注要求：
  ...
  4. 添加文字标注指向 URL 框："填写 https://你的平台域名"
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://你的平台域名",
...
**Windows CMD：**
```cmd
set ANTHROPIC_BASE_URL=https://你的平台域名
...
**Windows PowerShell：**
```powershell
$env:ANTHROPIC_BASE_URL="https://你的平台域名"
...
| `connection refused` 或超时 | 网络问题 | 确认能访问 `https://你的平台域名` |
...
- 标注要求：
  ...
  2. 用红色方框框住 Gateway Base URL 输入框，添加文字标注："填写 https://你的平台域名"
...
3. 填写以下信息：
   - **名称**：`ZeroCode`
   - **API Base URL**：`https://你的平台域名/v1`（注意末尾有 `/v1`）
   - **API Key**：`sk-你的Key`
4. 保存并切换到 **ZeroCode**
...
A：**不需要**。Claude Code 使用 Anthropic 协议，会自动在 base URL 后拼接 `/v1/messages` 等路径。只需填 `https://你的平台域名` 即可。

**Q：Codex CLI 的 `base_url` 后面要不要加 `/v1`？**

A：**需要**。Codex CLI 使用 OpenAI 协议，`base_url` 需要填 `https://你的平台域名/v1`。
