...
1. 访问 [CC-Switch 下载页](https://github.com/farion1231/cc-switch/releases)，下载你的操作系统对应的安装包（Windows `.msi` / macOS `.dmg`），安装后打开
...
3. 填写：
   - **名称**：`ZeroCode`（或任意名字）
   - **API Base URL**：`https://zerocode.kaynlab.com/antigravity`
...
- 标注要求：
  ...
  4. 添加文字标注指向 URL 框："填写 https://zerocode.kaynlab.com/antigravity"
...
> 💡 CC-Switch 还能同时管理 Codex、Gemini CLI 等多个工具的配置，也支持在官方 API 和 ZeroCode 之间一键切换。如果之前登录过 Anthropic 官方账号，建议先在终端执行 `claude /logout` 避免冲突。
...
Codex 是 OpenAI 官方推出的 AI 编程代理，能够理解代码库、编写和修改代码、运行测试，并帮助你完成复杂的编程任务。Codex 目前有 **CLI 命令行版** 和 **桌面版** 两种形态，它们都使用 OpenAI 的 Responses API 协议，通过 ZeroCode 平台可以直接调用 GPT 系列模型。

最重要的是：**Codex CLI 和 Codex 桌面版读取同一套本地配置文件**，也就是用户目录下的 `.codex/config.toml` 和 `.codex/auth.json`。因此不需要分别配置两遍，使用 **CC-Switch 一键配置 Codex** 后，CLI 和桌面版都会读取同一套 ZeroCode 配置。
...
Codex Desktop 是 OpenAI 官方桌面应用，支持 macOS 和 Windows。它适合希望使用图形界面、多会话窗口和本地项目入口的用户；如果你也安装了 Codex CLI，两者可以共用同一套本地 Codex 配置。
...
3. Linux 当前没有 Codex Desktop，Linux 用户请使用 Codex CLI

**安装注意事项：**

1. macOS 用户如果不确定芯片类型，点击左上角 Apple 菜单 → **关于本机**，查看是 Apple Silicon 还是 Intel
2. Windows 用户建议通过 Microsoft Store 安装，不要使用第三方重新打包的安装程序
3. 安装后先打开一次 Codex Desktop，按官方流程登录 ChatGPT 账号或使用 OpenAI API Key
4. 进入项目时选择本地项目目录，并使用 **Local** 模式，让 Codex 在本机项目中运行

> 重要：不要在 Codex Desktop 里到处找 ZeroCode 的 Base URL 配置入口。ZeroCode 接入推荐先通过 **CC-Switch** 写入 Codex 的本地配置文件。Codex Desktop 和 Codex CLI 会读取同一套 `.codex/config.toml` 与 `.codex/auth.json`，因此配置一次即可复用。

安装完成后，直接跳到 [4.3.1 CC-Switch — 图形界面一键配置（推荐）](#431-cc-switch--图形界面一键配置推荐)，先用 CC-Switch 添加并启用 ZeroCode Provider。配置完成后，重启 Codex Desktop，再进入项目测试。

#### Windows

在 Windows 上直接使用 PowerShell / CMD / Git Bash 安装即可：

```powershell
npm install -g @openai/codex
```

> 确保本机 Node.js 版本 >= 22。版本不够时，请先安装或升级 Node.js，再执行上面的安装命令。
...
Codex 使用两个配置文件（`config.toml` + `auth.json`）来设置 API 端点和认证信息。CLI 和桌面版读取的是同一套文件，因此推荐用 CC-Switch 统一生成和管理。
...
如果你在 [3.3.1](#331-cc-switch--图形界面一键配置推荐) 中已安装 CC-Switch，可以直接用它配置 Codex，操作与 Claude Code 类似：

1. 打开 CC-Switch，在左侧找到 **Codex**（或 **Codex CLI**）的配置区域
...
<!-- 
📸 截图 C2-1：CC-Switch 配置 Codex
- 截图内容：CC-Switch 界面中选择 Codex 配置区域
- 标注要求：
  1. 用红色方框框住左侧的 "Codex" 或 "Codex CLI" 菜单项
  2. 在表单中用红色方框框住 API Base URL，添加文字标注："注意末尾有 /v1"
  3. 用红色方框框住 API Key 输入框
-->

CC-Switch 会自动生成 `config.toml` 和 `auth.json`，无需手动编辑。由于 Codex CLI 和 Codex 桌面版读取同一个 `.codex` 配置目录，**这里配置一次，两端都会生效**。
...
> ⚠️ **注意 `base_url` 末尾的 `/v1`**：Codex 使用 OpenAI 协议，需要加上 `/v1` 路径前缀。这与 Claude Code 不同（Claude Code 不需要加 `/v1`）。
...
和 Claude Code 类似，ZeroCode 平台也提供了 Codex 的配置向导：

1. 登录 ZeroCode 后台 → **「API Keys」**
2. 点击目标 Key 右侧的 **「使用」** 按钮
3. 在弹窗中选择 **「Codex CLI」**（或 **「Codex CLI (WebSocket)」** 如需 WebSocket 模式；生成的配置同样适用于 Codex 桌面版）
