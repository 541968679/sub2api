...
After group/platform/`IsSchedulable` filters, selection calls `admitsScheduleUser`. If the request user has an **enabled** smart-schedule policy for `SmartScheduleLookupPlatform(account, hint, bundle)` **with at least one pool member**, that path is a closed pool + new quality gate + pair cooldown + pool-member pair cap, and the old account-side allow/deny/gate/cap for that user×platform are ignored. Bridge / Antigravity-group traffic on an OpenAI account looks up **openai** while AG is off/empty/missing, and **antigravity** only while AG is enabled with members; native OpenAI groups still look up `openai`. Scheduler eligibility for Claude→GPT bridge stays `account.Platform == openai` plus `ResolveClaudeGPTBridgeModel` — do not change the scheduler platform to antigravity. AG off/empty/missing keeps the openai closed pool (do not fail-open account-side just because AG is off). Once AG is on, a pool miss rejects and must not fall back to another pool. Otherwise it still calls `AdmitsScheduleUser` (identity `AllowsScheduleUser` plus optional live quality gate) then optional pair-cap checks. Do not fold pair limits, quality gates, or smart-schedule policy into `IsSchedulable()`, and do not write user policy onto shared account snapshots.
...
4. If identity-admitted and the account has a quality gate for this user, read Redis last-N \(Q_a\) (`account-quality:last-n:{accountID}`, projected onto `account-quality:live:{accountID}`) merged with resume HASH `account-quality:resume:{accountID}`. Breach excludes only this pair (clear sticky, reselect). Cache miss / nil stats / unconfigured or under-sampled metrics fail open. An active resume grace also admits. Quality block is not pair-full and must not emit `WaitPlan`. Pair cooldown stays on \(Q_{a,u}\).
5. If admitted and the resolved pair cap is `>= 1`, skip the candidate when the live Redis pair count is already `>= N`. After the account slot is acquired, acquire `concurrency:account_user:{accountID}:{userID}:{platform}` when the cap is `>= 1` **or** the user is an enabled smart-schedule closed-pool member (count-only when cap is 0/null). A real cap reject releases the account slot, excludes the account, and reselects (`pair_full`). Uncapped occupancy writes never `pair_full` and never use 999 or `account.concurrency` as a pair limit. Pair-full never emits `WaitPlan` / 429 by itself. Same-request acquire `pairFull` is recorded in `pairFullIDs` so Layer 3 / routing wait cannot queue that account from a stale snapshot. After `AcquireAccountSlotWithWaitTimeout`, handlers call `AttachPairSlotAfterAccountWait`; `pairFull` releases the account slot and reselects. Users not in a closed pool still skip pair writes when there is no real cap.
...
| Service | `backend/internal/service/ratelimit_service.go` | Maps upstream errors to account state, temporary unschedulable windows, and rate limits. OAuth fleet soft 429 classifies before `tryTempUnschedulable`. |
| Service | `backend/internal/service/oauth_fleet_soft_429.go` | Applies predicate, soft/hard classify, Redis persist, scheduler merge (skip on sticky / `previous_response`). |
| Repository | `backend/internal/repository/oauth_fleet_soft_429_cache.go` | Redis `oauth-soft-429:{accountID}` SET/SCAN. Fail-open. |
...
| `usage_logs.true_first_token_ms` | True first-token: first useful/non-preamble event (`openAIStreamDataStartsClientOutput`). Empty `output_item.added` shells, retryable `error` frames, and control frames (`keepalive` / `ping` / `heartbeat` / `codex.rate_limits`) do not count; visible deltas / non-empty added items do. | Account scheduler `ReportResult`, account-list quality TTFT, ops TTFT aggregations. `COALESCE(true_first_token_ms, first_token_ms)` for historical rows. |
...
|--------|------------------|----------------------------------|
| Off (default) and user not in allowlist | Buffer `response.created` / `response.in_progress`, empty `output_item.added` shells, control frames (`keepalive` / `codex.rate_limits`), and retryable `error` frames until visible output. Native `:\n\n` keepalive comments are also held until visible output. Native preamble stays in an in-memory `pendingLines` slice — **not** the 4KiB `bufio.Writer` on `ResponseWriter` (a `response.created` ≥ 4KiB write-through commits HTTP 200). | Still possible: HTTP 200 is not committed to the client yet, so JSON error rewrite / account failover can still run |
...
`response.failed` is never treated as a preamble commit. Empty `output_item.added` / `content_part.added` / `reasoning_summary_part.added` shells, control frames (`keepalive` / `ping` / `heartbeat` / `codex.rate_limits`), retryable `type=error` frames, and native `:\n\n` keepalive comments also stay unflushed so pre-output account failover can still run. Flushing any of those before visible output commits HTTP 200; NewAPI then records the later SSE error as a successful empty-output or ~1-token completion. Native `handleStreamingResponse` must not write uncommitted lines into `bufio.NewWriterSize(ResponseWriter, 4KiB)`: Go's `bufio.Writer` writes through when the payload is ≥ the buffer, so a large Codex `response.created` (tools/instructions) commits the stream even when `Flush()` is skipped. Passthrough already used `pendingLines`; native now does the same until `commitDownstream`. If the stream already committed visible output, capacity-shed codes `server_is_overloaded` / `slow_down` are rewritten to `server_error` before the client sees them (message text is unchanged) so Codex retries instead of showing "Selected model is at capacity". Claude-GPT visible-output buffering is unchanged.
...
GET /v1/models with API key
  ...
  -> auth snapshot must already contain groups.models_list_config
  -> if the platform has a curated discovery list and models_list_config is enabled:
...
| Group custom models list | `groups.models_list_config` only customizes `GET /v1/models` output. On catalog platforms the saved list is the display source of truth even when the CCS import picker is enabled, and it may include IDs outside the curated catalog. The API-key auth snapshot (v13) and `GetByKeyForAuth` must carry the column or cached keys ignore the setting. Stale full-default OpenAI lists are still expanded to the current catalog. It is ignored by scheduling and billing paths. |
...
- **R2 first useful frame**: after 2xx headers, wait for `openAIStreamDataStartsClientOutput` (not `response.created` / `in_progress`).
  - Uncommitted downstream → `UpstreamFailoverError` 502 + **Ops** marker `openai_first_useful_frame_timeout` (silent account switch).
  - Already committed (flush-preamble, keepalive `:`, or any write) → write a **client** 502 (`Upstream service temporarily unavailable`) and return `upstream response failed: openai_first_useful_frame_timeout…` for Ops. **Do not switch accounts.** Do not leave the hop to stream-interval timeout (`in_progress` keepalives reset that clock).
- Markers also feed schedule last-N when Recovered; do not flip global `schedule_use_failover_error_rate`.
- **Client vs Ops split**: `openai_header_wait_timeout` / `openai_first_useful_frame_timeout` (and `waited_ms=`) are Ops-only. `ResponseBody.error.message`, Anthropic `event: error`, and OpenAI SSE `error.message`/`error.code` must use `OpenAIWaitTimeoutClientMessage()` (`Upstream service temporarily unavailable`). Keep the marker in `event.Message`, `RawUpstreamBody`, and `error.Error()`. Do not put the generic 502 sentence into `event.Message` — `recordOpsUpstreamAttempt` drops generic Ops messages. Claude-GPT bridge `handleAnthropicFailoverExhausted` replays `ResponseBody`; that is why the marker must never live in the client wrapper.
...
- **GPT-5.6-terra / GPT-5.5 may put visible text only on `content_part`**: production `openai_messages.stream_completed_without_visible_output` on Claude→GPT Messages bridge (2026-08-17) had `response.content_part.added/done` + `response.output_text.done` and **zero** `output_text.delta`. Terminal `message` text was empty. The converter must harvest `part.text` (and `type=text` as well as `output_text`); treating thinking-only / empty completed as a 502 without reading content_part drops a real assistant reply. Diagnostic field: `content_part_text_bytes`.
...
- **Codex remote compact V2 is a client-side count, not an HTTP error**: `remote compaction V2 expected exactly one compaction output item, got 0 from 2` is Codex Desktop/CLI validating `ResponseItem::Compaction` after the stream ends. Third-party API-key upstreams typically return `reasoning` + `message`. `got 0 from 0` means the client never deserialized a Compaction item: Codex requires non-empty `encrypted_content` (not `Option`), so a `summary_text`-only synthetic item is dropped. Do not “fix” this by live-passthrough then appending a third item (Codex Fatals on multiple compaction), by omitting `encrypted_content`, or by synthesizing a response without expanding the next-turn historical item (silent amnesia — Codex reserializes only `id` + `encrypted_content`). Mint `sub2api_compact_v2:` + summary on API-key synthesize only; never invent official-looking ciphertext for OAuth. Official merged #5641 only changes V2 routing; it does not produce a compaction item. Unmerged official #5651 is CC-only and misses this fork’s default native `/v1/responses` path.
- **Official Codex stale encrypted compaction is recovered on 400, not pinned**: Official Codex `/v1/responses` may replay `type=compaction` / `compaction_summary` with account-bound `encrypted_content` (`cmp_*`). After upstream 400 `invalid_encrypted_content`, the existing HTTP/WSv2 same-request retry drops those encrypted items (and still strips reasoning ciphertext) then retries once on the current account. First hop still sends the client body intact. Unencrypted compaction stays; `context_compaction` is not dropped. This does not pin the conversation to the issuing OAuth account, does not rewrite compact v2 synthesize, and does not change scheduler/sticky. Overlay of Wei-Shaw `fe2172586` on the existing sanitizer in `openai_gateway_service.go`.
