...
5. If admitted and the resolved pair cap is `>= 1`, skip the candidate when the live Redis pair count is already `>= N`. After the account slot is acquired, acquire `concurrency:account_user:{accountID}:{userID}` when the cap is `>= 1` **or** the user is an enabled smart-schedule closed-pool member (count-only when cap is 0/null). A real cap reject releases the account slot, excludes the account, and reselects (`pair_full`). Uncapped occupancy writes never `pair_full` and never use 999 or `account.concurrency` as a pair limit. Pair-full never emits `WaitPlan` / 429 by itself. Users not in a closed pool still skip pair writes when there is no real cap.
