...
4. User admission: enabled smart-schedule with a non-empty pool is a closed allow-list + pair cooldown + pool pair-cap. The pool key is `SmartScheduleLookupPlatform` (OpenAI + Claude-GPT bridge or AG-group → `antigravity`; native OpenAI groups stay `openai`). Disabled or empty-enabled fail-opens to account-side allow/deny/gate/cap and never falls back to another platform pool.
