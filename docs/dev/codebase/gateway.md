...
- **Replayed custom tool item IDs can fail across Responses paths**: Chat Completions -> Responses fallback currently emits `custom_tool_call.id` through the generic `generateItemID()` helper as `item_<24 hex>`. A later long-running client turn may replay that output into `/responses`; the OpenAI OAuth transform enables reference preservation for tool continuation and currently strips invalid IDs only from `message` items, so the generic custom-tool ID reaches ChatGPT/Codex unchanged. The upstream validates custom tool call item IDs with the `ctc` namespace and returns HTTP 400 such as `Invalid 'input[N].id': 'item_...'. Expected an ID that begins with 'ctc'.` This is a protocol-loop defect, not an account/quota/model-availability failure. A complete fix must make bridge output IDs type-aware (`ctc_` for custom calls, `fc_` for function calls, native namespaces for other validated item types) and defensively omit invalid replayed IDs before both OAuth and API-key upstream forwarding; `call_id`, not a fabricated item ID, remains the tool call/output pairing key. Cover streaming and non-streaming bridge output plus replay sanitization together.
