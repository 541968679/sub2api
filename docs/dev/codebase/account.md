...
3. **Smart pool** — incremental `GET/POST/DELETE /admin/accounts/:id/smart-schedule-memberships|members` plus existing `smart-schedule-resume` / `smart-schedule-admission-batch`. Add does not auto-enable the user×platform (`enabled=false` if the policy row is new). Remove last member disables that user×platform. Pause ≠ remove. OpenAI accounts get `openai` + `antigravity` tabs; other platforms only their own tab. Always send `platform` when switching state. Batch six-state writes only the selected members on that tab.
