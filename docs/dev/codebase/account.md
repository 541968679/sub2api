...
Admin writes this only on the user smart-schedule page (`UsersView` smart-schedule column → `/admin/users/:id/smart-schedule`). The users table column shows enabled platforms and pool counts via `POST /admin/users/smart-schedule/summaries`. The detail page header is one `UsersView` list row (`AdminUserListRowTable`: email/id/username/role/groups/subscriptions/balance/usage/concurrency/smart-schedule/TTFT/success/status/last active/last used/created), then a left user-policy panel plus a right account-pool panel. User TTFT/success use `POST /admin/users/quality-stats/batch`; today/30d usage uses `POST /admin/dashboard/users-usage`. Do not derive prices from cost/tokens. The per-platform enable control is a header switch that PUTs immediately; quality/cooldown/pool-cap edits still use Save. An empty pool cannot be turned on. Removing the last member turns the platform off and persists that disable. Storage is `user_smart_schedule_policies` + `user_smart_schedule_accounts` (migration 202), not `account_schedule_users`. One policy row per `(user_id, platform)` for `anthropic` / `openai` / `gemini` / `antigravity` / `grok`.
...
The pool table reuses the account-list `DataTable` cells: account capacity (inline max + `AccountCapacityCell`), pair occupancy/cap, clickable quality (`AccountStabilityDialog`), today stats, groups, usage windows, schedulable/fallback toggles, priority, upstream rate, last used, and the account action set (edit / stability / usage / errors / more). Pool remove is “remove from pool”, not account delete. Pair-cap edits stay draft until Save; account concurrency / priority / upstream rate / schedulable / move-to-top save immediately via the account update API. **自动排序** reorders current-platform members by admission → pair headroom → account concurrency → existing priority → LRU/`id`, then immediately writes distinct `accounts.priority` values (lower number = more preferred, same as Claude / same-rate OpenAI). It reassigns this pool’s existing priority slots (spreading duplicates upward) so the fleet’s other accounts keep their numbers; it does not change membership, is not Save-gated, and does not sort by `upstream_rate_multiplier`. Empty/null pair cap means no extra cap and pair Redis slots are not acquired — the badge shows “不额外限制”, never `account.concurrency`. GET hydrates `current_concurrency` only when cap ≥ 1, plus read-only `cooldown_until` from HASH `smart-schedule:cooldown:{accountID}` field `u:{userID}`. `default_platform` prefers an enabled platform, then the most recently updated / largest pool. The page opens that tab (zuoge85 lands on openai). The pool toolbar is three distinct regions: **add** (quick typeahead + filtered-add dialog + one-click currently-scheduling presets), **in-pool filters** (account-list chrome: search / type / schedulable / admission), and **bulk** (select page / select filtered / clear, batch remove-from-pool, batch/all pair cap — never account delete). The filtered-add dialog reuses `AccountTableFilters` (platform locked to the active tab) plus schedulable / currently-scheduling / proxy, previews candidates not already in this platform pool, then add-selected or add-all-matching. Membership edits stay Save-gated. Pool filters cover type, account schedulable, and admission (`selectable` / `cooling` / `pair_full` / `stopped` / `quality_blocked`); batch remove/apply-cap run on the current filtered selection and stay Save-gated. Auto-refresh copies the account-list dropdown and skips overwriting dirty membership/caps. Sorting is client-side, default-grouped by `platform_type`; after auto-sort the table switches to `priority asc`. Column settings reuse the account-list visibility / order / width pattern, persisted under `smart-schedule-pool-*` keys so they do not share the account list layout. Select, name, and actions stay pinned.
...
| 账号质量列（首字/成功率） | 列表默认显示最近 **15 分钟**滚动窗口的平均 TTFT 与成功率；成功与首字来自 `usage_logs`，失败来自 `ops_error_logs`（status≥400，排除 count_tokens）。列隐藏时不请求；`POST /admin/accounts/quality-stats/batch` + 30s 缓存。不参与调度/熔断。账号管理页点击单元格打开 `AccountStabilityDialog`（24h 曲线 + 账号硬关闭表单）；曲线默认隐藏 p95，左轴按当前可见 TTFT 序列缩放（显示 p95 时若会压扁 p50 则按 `max(p50)*2.5` 裁剪）；用户页只读 | `account_quality.go`, `usage_log_repo.go`, `AccountsView.vue`, `AccountQualityCell.vue`, `AccountStabilityDialog.vue` |
