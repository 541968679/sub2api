...
When `credentials.newapi_access_token` (system access token, not `sk-`) and `credentials.newapi_user_id` (≥ 1) are both set, `ProbeUpstreamBalance` calls `GET {origin}/api/user/self` first (`Authorization: Bearer <access_token>`, `New-Api-User: <id>`). Success writes extra snapshot `source=newapi_user_self`, remaining = `quota / quota_per_unit`, used = `used_quota / quota_per_unit`, `unlimited=false`. `quota_per_unit` still comes from `/api/status` (default 500000). A `data.id` mismatch, 401/403, HTML, or non-JSON fails the wallet step with a short error (never the token) and falls back to the existing order: `/v1/usage` → `/api/usage/token` → credit_grants → subscription. Do not treat subscription `$100000000` as the wallet.
