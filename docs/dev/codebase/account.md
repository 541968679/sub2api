...
> 策略真源见 [scheduler.md](./scheduler.md)。本节保留账号面契约，不替代手册。
...
A user may sit on allow, deny, a cap, and a quality gate at once. Runtime identity priority is deny → allow-whitelist miss → then optional quality gate → explicit pair cap N≥1 → default unrestricted (account + user global concurrency only). `0` / null / omit means no pair cap. A quality gate is enabled when p50 or success rate is set; samples/condition are modifiers. Unconfigured metrics and under-sampled windows are not judged. Account-level \(Q_a\) is last-N (site-wide `account_quality_window_n`, default 20) in Redis `account-quality:last-n:{accountID}`; ingest projects `account-quality:live:{accountID}`. Selection `Get` prefers last-N. Cache miss fails open. Pair cooldown stays on \(Q_{a,u}\). Manual **立即恢复** grace lives in a separate HASH `account-quality:resume:{accountID}` and must survive even when last-N is missing. The 5-minute tick must not `Replace` live from a 15-minute SQL window. A quality block is admission fail for that pair only: clear sticky and reselect. It must not change `schedulable` / `TempUnschedulableUntil` or fold into `IsSchedulable()`. Restore-default writes four empties. Bulk edit may overwrite allow and/or deny (checkbox per list) and never writes caps or quality gates. Quality-gate **保存模板 / 应用模板** reuse the same site-wide hard-close threshold KV; the template is not bound to a user. **立即恢复** keeps the gate and writes a 15-minute resume-HASH grace so the current window cannot immediately exclude that pair again.

The Edit Account modal no longer edits those leftover lists. Save omits `allow_user_ids` / `deny_user_ids` / caps / quality gates so an edit of name/groups cannot wipe them. The lists still apply on the unpooled hot path.

## Account-side three-layer schedule

The **账号调度** card in `EditAccountModal` is one account × many users:

1. **Master** — existing `accounts.schedulable` (`POST /admin/accounts/:id/schedulable`). Off ⇒ nobody can select it.
2. **Public pool** — `accounts.extra.public_schedulable`. Missing / not `false` = in the public pool (same as today). Explicit `false` excludes only users without an `EnabledPolicy`. In-pool users ignore this flag. Do not reuse `schedulable` or `fallback_only`. `filterSchedulerExtra` keeps the key; old snapshots that omit it fail open. Turning public off does not clear sticky. Dedicated `POST /admin/accounts/:id/public-schedulable` writes the key; `UpdateAccount` must keep `public_schedulable` and `public_schedule_quality` when the edit form replaces `extra`, same as `quality_hard_close`. The edit form strips those dedicated keys before save so a stale extra payload cannot overwrite them.
3. **Smart pool** — `GET /admin/accounts/:id/smart-schedule-memberships` lists every user with a `user_smart_schedule_policies` row on that platform, with `in_pool` when this account is a member. Incremental `POST/DELETE .../members` and `POST .../members-batch` `{action: add|remove}` plus `smart-schedule-resume` / `smart-schedule-admission-batch`. The 账号调度 table has two clickable columns: in/out of pool (membership) and in-pool admission (six-state dropdown on the chip). The user pool page uses the same chip dropdown; the actions-column switch is gone. Add does not auto-enable the user×platform (`enabled=false` if the policy row is new). Remove last member disables that user×platform. Pause ≠ remove. OpenAI accounts get `openai` + `antigravity` tabs; other platforms only their own tab. Always send `platform` when switching state. Batch six-state writes only selected **in-pool** members on that tab.

### Admin list order (display only)

`extra.list_order` (higher first) is the account-list pin key. `MoveAccountToTop` and page-local drag `PUT /admin/accounts/reorder` also set `extra.list_pinned=true`. Create still writes unix-ms `list_order` so new rows float above older pins; that is **not** a manual pin.

`PUT /admin/accounts/reorder-auto-sort` writes reserved-band `list_order` `N…1` for the current-filter set (cross-page). It skips `list_pinned`, never uses now-ms respread, and never writes `accounts.priority`. The Accounts toolbar **排序（手动）** / **自动排序** reuse the smart-schedule pool comparator with public-pool field mapping (six-state / availability, `fallback_only`, `current_concurrency`, `EffectiveUpstreamRate`). Auto-sort follows auto-refresh; it does not start its own timer. After a write, the page clears the list ETag and silently merges the current page (`skipAutoSort`); it must not `reload()` (that would set `loading` and jump to page 1) and must not enter the 15s mutation silent window (that hijacks the countdown). Drag / edit still use that silent window. This does not change public-schedule selection. See `.trellis/tasks/08-29-public-pool-auto-sort/prd.md` and `.trellis/tasks/09-01-public-pool-auto-sort-ux/prd.md`.

Hot-path details stay in [scheduler.md](./scheduler.md) and `.trellis/spec/backend/account-user-schedule.md`. The account list `user_schedule` column is unchanged and still default-visible: all four empty shows `—`; otherwise union chips with allow/deny tags, a quality-state chip (`质量` / `已停` / `已恢复`), plus an inline pair-cap mark (`user_concurrency_patch`) and inline quality patch (`user_quality_gate_patch`). **立即恢复** shows `已恢复` and force-admits the pair. Clicking `已恢复`, or waiting 15 minutes, returns to `质量` and starts a new accumulation window (`quality_window_until`). List `schedulable` stacks the schedule toggle and fallback-only toggle. `concurrency` keeps the inline max editor plus the remaining capacity badges. `quality_ttft` shows p50, display-only p95, schedule success rate (terminal last-N `OK`), failover contrast (`1 - failover_error_rate` from the same last-N outcome FIFO including Recovered hops), k/N, and the same K/C line as smart-schedule pair quality cells (K/C use the public-schedule site knobs on the \(Q_a\) last-N window). Recovered hops do not enter user last-N. First-token and duration stay on the hop `Forward()` clock (same number for usage, last-N, pair, and schedule). `public_quality` is the account-level six-state chip (selectable / cooling / probing / paused / resumed / pinned, plus will_cool) with a manual switcher; it is not folded into `IsSchedulable()`. The Accounts toolbar site card (`PublicScheduleQualityGlobalCard`) edits the same EvalQuality groups as the smart-schedule pool (threshold / probe / selectable / cooldown; no probe v2 and no probe concurrency). It stays height-matched to the left filter/action band: one top bar (threshold + cooldown) and two cards below (probe | selectable). Scheduler evaluation is documented in [gateway.md](./gateway.md#account-user-schedule-filter).

## Unbind subscription groups by billing rate

Ops action on the account list toolbar (next to 按筛选批量编辑). Not a scheduler rule and not part of `IsSchedulable()`.

```
POST /api/v1/admin/accounts/unbind-subscription-groups-by-rate
  -> adminService.UnbindSubscriptionGroupsByRate
  -> ListAllWithFilters(platform, "", "", "", 0, "")
  -> per-account BindGroups(id, keepIDs)
```

- Compare `Account.BillingRateMultiplier()` (nil/negative → `1.0`; explicit `0` stays `0`). Do **not** use `upstream_rate_multiplier`.
- Match is strict `>` the threshold. Equals does not match.
- Drop groups where `Group.IsSubscriptionType()` (`subscription_type == subscription`). Keep standard, unknown, or a join row whose Group edge is nil.
- Preview (`dry_run` missing or true) never writes. Apply re-scans live DB; do not replay a stale preview ID list.
- Write is per-account `accountRepo.BindGroups`. Do not call bulk-update `GroupIDs` (that assigns one shared list).
- Skip accounts with no subscription group. Skip a zero-group remainder unless **允许改成无分组**, which then writes `BindGroups(id, [])`.
- Does not write leftover allow/deny, smart-schedule membership, `schedulable`, or `public_schedulable`. `BindGroups` already enqueues the existing groups-changed scheduler outbox.
...
- Account-list bulk bar **按上游倍率选中** pages the current list filters (same as 跨页全选), then keeps ids whose `EffectiveUpstreamRate()` is strictly above or below a threshold. Equals is not selected. Missing / negative rates use the type default (`oauth`/`apikey` → `0.15`). This only writes the checkbox selection; add/remove groups and Claude-GPT bridge stay on existing bulk edit. Do not reuse `rate_multiplier`. The billing-rate unbind dialog is a separate action and still compares billing rate.
...
Session sticky defaults to keeping the pin (prefix cache). Redis value is `{accountID}` or `{accountID}|o` (overflow). A pin is overflow when this session had to take a higher `EffectiveUpstreamRate` than the cheapest **admitted** peer (full/unavailable) or than the sticky it was forced off. Overflow may clear **once** when a strictly cheaper admitted, non-pair_full peer has `LoadRate<100`; the new pin is overflow=false and does not chase a still-cheaper later peer. If the current pin cannot serve (paused / cooling / quality / pair_full / account full / sticky_escape), switch immediately; cheap-full does not WaitPlan. `previous_response` is never cleared for a cheaper peer. Legacy integer-only keys are overflow=false. See [gateway.md](./gateway.md#upstream-rate-overlay).

## OAuth fleet soft 429

Settings KV `oauth_fleet_soft_429_settings` (Admin Gateway tab, self-save). Factory `DefaultOAuthFleetSoft429Settings().Enabled=false`; empty/missing/bad KV is OFF (do not copy 529 overload cooldown’s empty-KV=on). Resolution: `accounts.extra.oauth_fleet_soft_429` true/false/unset → global KV → code Default\*. Unset inherits. `true` is a canary even when global is off. Override lives in **extra**, never credentials (OAuth refresh replaces the credential map).

Soft 429: do **not** write `rate_limit_reset_at` / `temp_unschedulable_until`. This request still adds the id to `failedAccountIDs`; Redis `oauth-soft-429:{accountID}` (TTL `ttl_seconds`, default 20, 5–300) is a scheduler exclude only. It must not enter `IsSchedulable()` or Admin「可调度」. Hard affinity (existing sticky **binding** / `previous_response`) skips layer-2; a generated `sessionHash` without a binding does not. Redis read/write fail-open.

Hard still persists as today: Anthropic official 5h/7d window, Codex `used_percent>=100`, quota death / `额度已用完`, `hard_body_codes`, `long_reset_policy=hard` or threshold. Built-in hard beats the admin code table. Classify before `tryTempUnschedulable` so a wide “rate limit” rule cannot steal a soft 429. Do not open `IsPoolMode()` to OAuth. Do not same-account-retry OAuth 429. OAuth 401 still invalidate + temp unsched.
...
When `credentials.newapi_access_token` (system access token, not `sk-`) and `credentials.newapi_user_id` (≥ 1) are both set, `ProbeUpstreamBalance` calls `GET {origin}/api/user/self` first (`Authorization: Bearer <access_token>`, `New-Api-User: <id>`), then `GET {origin}/api/subscription/self` with the same headers. Wallet remaining = `max(0, quota / quota_per_unit)`; subscription remaining = sum of `max(0, amount_total - amount_used) / quota_per_unit` for `data.subscriptions[]` rows with `status=active` and `end_time > now` (do not use `all_subscriptions`). Display `upstream_balance_usd` is wallet + subscription. Extra also stores `upstream_balance_wallet_usd` and `upstream_balance_subscription_usd` (the latter only when remaining > 0). Source is `newapi_wallet_subscription` when the subscription call succeeded, else `newapi_user_self`. Used still comes from wallet `used_quota`. `unlimited=false`. `quota_per_unit` still comes from `/api/status` (default 500000). A `data.id` mismatch, 401/403, HTML, or non-JSON fails that step with a short error (never the token). If both wallet and subscription fail, fall back to `/v1/usage` → `/api/usage/token` → credit_grants → OpenAI billing subscription. Do not treat OpenAI billing `$100000000` as the wallet or New API subscription. A later wallet-only success clears a stale subscription extra key.

Empty access-token input on edit keeps the saved token. **清除钱包凭据** deletes both keys and returns the account to token-quota probing. Unconfigured accounts keep the pre-change path (unlimited tokens can still show remaining `$0` + `unlimited=true`). Lite list (`liteAccountCredentials`) only keeps `plan_type` / `email` / `subscription_expires_at`, so the access token never appears on `GET /admin/accounts?lite=1`. `GET /admin/accounts/:id/usage` still writes the extra snapshot (`shouldRefreshUpstreamBalance`, 6 min; `force=true` bypasses TTL). Wallet success resumes burn sampling because `unlimited=false`.

Admin create/edit surfaces the pair only for openai/anthropic `apikey`. Per-account credentials; v1 does not share one token across a host.

## User × platform smart schedule

Admin writes pool policy/quality/sort on the user smart-schedule page (`UsersView` smart-schedule column → `/admin/users/:id/smart-schedule`). Account-side add/remove/state for **this account's members** lives on the Edit Account **账号调度** card (incremental membership APIs; not replace-all). The users table column shows enabled platforms and pool counts via `POST /admin/users/smart-schedule/summaries`. In-page chrome is a single row (`返回用户列表` + `pageDescription`); the long title/subtitle block is gone (`AppHeader` still uses `titleKey`). Below that is one `UsersView` list row (`AdminUserListRowTable`: email/id/role/groups/subscriptions/balance/burn-rate/usage/concurrency/smart-schedule/schedule-pnl/TTFT/success/status/last active/last used/created/actions — no username). The actions column matches UsersView (edit / usage / errors / toggle status / more). Header burn rate reuses the UsersView `UserBurnRateCell` and `POST /admin/dashboard/users-burn-rate` (trailing 5m `actual_cost` → `$/h`; do not derive from cost/tokens). First-paint `loading` waits only for `getSmartSchedule` + pool member details (`lite=1` ids list), not the platform candidate page-walk. AG pool hydrate must fetch those ids **without** `platform=antigravity` (ids-only, or merge AG+OpenAI): the accounts list intersects `platform` with `account.Platform`, so an OpenAI dual-member (loveapi-style) vanishes after add/save/refresh even though GET/PUT kept it. Candidates load lazily on add-search focus, filtered-add open, or one-click “add scheduling” (those buttons stay clickable at count 0 so they can trigger the load). `loadAll({ pickPlatform: true })` skips the `activePlatform` watch reload. Manual/auto refresh is silent: keep the current DOM, spin the refresh icon, patch pool + visible header extras (quality/usage/concurrency/subscriptions/burn-rate/smart-schedule from drafts); do not set full-page `loading`/`userLoading`. `statsLoading` only spins quality/today cells; the pool DataTable uses virtual scroll and is not skeleton-replaced. Account `GET ?lite=1` skips schedule_users/quality-live hydrate, window-cost N+1, and credential secrets. Pool **编辑** must `GET /admin/accounts/:id` before opening `EditAccountModal`; passing the lite row leaves credentials, extra mappings, and user-schedule empty. The modal only re-hydrates when it opens or the account id changes, so a same-id lite→full swap would not refill the form. Then two stretched side-by-side cards on `lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]` (not a locked ~28rem left chimney): left quality-threshold editor is three compact rows (email + enable + platform tabs; 3-col inline p50/success/N首字/N成功率/condition/cooldown; copy/save), right is pool title + horizontally dense add+filter. The pool table and bulk bar sit full-width under those cards. User TTFT/success use `POST /admin/users/quality-stats/batch`; today/30d usage uses `POST /admin/dashboard/users-usage`. Do not derive prices from cost/tokens. The per-platform enable control is the quality-card switch that PUTs immediately; quality/cooldown/pool-cap edits still use Save. An empty pool cannot be turned on. Removing the last member turns the platform off and persists that disable. Storage is `user_smart_schedule_policies` + `user_smart_schedule_accounts` (migration 202; `sort_order` in 204; probe concurrency in 208; latency-gate columns in 214+215, repaired by 216 if goose Down already ran), not `account_schedule_users`. One policy row per `(user_id, platform)` for `anthropic` / `openai` / `gemini` / `antigravity` / `grok`.

When `enabled=true`, that user×platform is a closed allow-list: pool miss rejects; in-pool ignores the account-side allow/deny/gate/cap for that pair. Quality still reuses `EvaluateAccountQualityHardClose` and the live 15-minute cache. A breach writes pair cooldown `smart-schedule:cooldown:{platform}:{accountID}` HASH field `u:{userID}` via `HSETNX` (no extend, not `TempUnschedulable`). The same OpenAI account may sit in both the openai pool and the antigravity pool (`PK (user_id, platform, account_id)`, migration 211). Occupancy, cooldown, probe, pin, pair-quality, pair cap, pair 豁免期 (`smart-schedule:resume:{platform}:{accountID}`), and `ObservePairCompletion` are isolated by `(user, account, platform)`. Track A `account-quality:resume` is not the smart-schedule grace. Bridge / Antigravity-group traffic on an OpenAI account looks up **openai** while AG is off/empty/missing, and **antigravity** only while `EnabledPolicy(antigravity) != nil` (`SmartScheduleLookupPlatform` reads the user policy bundle). Do not fail-open to account-side just because AG is off. Once AG is on, a pool miss rejects and must not fall back to the openai pool. Native OpenAI groups still look up `account.Platform=openai`. Scheduler eligibility for Claude→GPT bridge stays `account.Platform == openai` plus `ResolveClaudeGPTBridgeModel`; do not change the scheduler platform to antigravity. Pool **切换状态** (`POST /admin/accounts/:id/smart-schedule-resume` `{user_id, state, platform?}`) writes Redis first, then DB `paused`. Omitted `platform` on the account page only touches the `account.Platform` row. From the pool page, pass the active tab: `paused` (stay in pool + clear cooldown/resume/probe/pin; no implicit unpause), `cooling` (`HSET` overwrite + clear resume/probe/pin), `probing` (clear cooldown + zero windows + probe mark + `probe_cap`; no `u:`/`w:`; clear pin), `resumed` (`HDEL` + resume grace + clear probe/pin; omitted `state` is this, never probing or pinned), `selectable` (`HDEL` + zero windows + `ClearUserResume`, no `w:`, clear probe/pin), or `pinned` (长期豁免: clear cooldown/probe, `ClearUserResume`, no `u:`/`w:`, no `MarkUserResume`, keep windows, `HSET` `smart-schedule:pinned:{platform}:{accountID}` `u:{userID}` with no TTL; full member cap; never evaluate / `StartCooldown` until admin leaves). `SetCooldown` returns HSET errors so a failed cooling write does not unpause. Pause patches the user Redis bundle in place (`ApplyMemberPaused`) instead of only `DEL`. The switch live-state uses `member.paused` even when the chip is `stopped`; a session-local pause overlay survives a stale auto-refresh. `paused` is `user_smart_schedule_accounts.paused` (migration 207); PUT replace-all keeps it for remaining members. Hot-path `StartCooldown` stays `HSETNX`. A paused pair is skipped after the pool hit and before cooldown; it is not `accounts.schedulable` and must not fold into `IsSchedulable()`. Pair cap comes from the pool member, not `account.UserConcurrency`. GET/PUT responses hydrate read-only `current_concurrency` from Redis pair slots (`GetAccountUserConcurrencyBatch`) for every pool member (capped and uncapped), plus `cooldown_until` from the pair cooldown HASH; writes ignore those fields. Hydrate is admin-read only and does not acquire pair slots or change scheduling. Each member also carries `sort_order` (nullable INT on `user_smart_schedule_accounts`, migration 204): this is **pool display order only**. Empty/null keeps relative id order until the first auto-sort or 移到顶部. The view includes `default_platform` so the admin page opens the relevant tab. Empty enabled saves are rejected; removing the last member (admin pool save **or account delete**) turns the platform off and returns to the legacy account-side rules. Soft-delete does not fire FK CASCADE, so `account_repo.Delete` must delete `user_smart_schedule_accounts` for that `account_id` in the same transaction, persist `enabled=false` when a `(user_id, platform)` pool becomes empty, then `DEL smart-schedule:user:{userID}` and clear every platform’s cooldown/probe/pin/quality for that `(account, user)` (`ClearCooldownAllPlatforms`). GET omits members that `GetByIDs` cannot load. PUT strips those already-in-pool missing IDs so a poisoned draft can save; newly added unknown IDs still return `SMART_SCHEDULE_UNKNOWN_ACCOUNT`. An enabled row with zero members is treated as disabled on the hot path so the user is not fail-closed. Copy-from-platform (`POST .../:platform/copy` `{from_platform}`) copies enable/thresholds/cooldown/probe concurrency settings only (not the pool, not account-quality N into `probe_concurrency`). Copy-from-user (`GET .../:platform/copy-from-preview?source_user_id=`, `POST .../:platform/copy-from` `{source_user_id, source_revision, slices}`) replaces the **current platform** pool with the source user's usable members and source `paused` when `slices.pool` is on (whole-pool replace, not union). Pause follows the source on overlap; leftover target pause is not restored (`ReplacePlatformWithMemberPaused`). Default slices: pool+pause, pair concurrency, `sort_order`. Off by default: thresholds/cooldown and platform `enabled`. Unchecking pool disables concurrency/sort. Empty usable source + pool is rejected (`SMART_SCHEDULE_COPY_EMPTY_SOURCE`). Confirm re-reads the source; stale revision is `409 SMART_SCHEDULE_COPY_STALE`. Does not copy Redis pin/cooldown/probe/resume/occupancy/PnL. Same-user platform copy is unchanged. Closed-pool lookup uses `SmartScheduleLookupPlatform` (AG off/empty/missing keeps openai for bridge/AG-group OpenAI; AG on with members uses antigravity; native OpenAI groups stay openai). The AG tab may hold OpenAI members; other tabs stay platform-locked. Do not copy this policy onto scheduler snapshots.

The pool table reuses the account-list `DataTable` cells: account capacity (inline max + `AccountCapacityCell`), pair occupancy/cap, clickable quality (`AccountStabilityDialog`), today stats, groups, **调度利润** (`SmartSchedulePnlCell`, this user × this account today PnL; not `AccountUsageCell`, no 7-day PnL window; API-key rows keep readable upstream balance from `extra.upstream_balance_usd`, OAuth rows replace that line with the cached 7-day quota bar from extra), schedulable/fallback toggles, **池内顺序** (`user_smart_schedule_accounts.sort_order` only), **调度优先级** (live `accounts.priority`, same field as Account Management), upstream rate, last used, and the account action set (edit / stability / usage / errors / more). When a non-oauth row has readable `extra.upstream_balance_usd` (or usage `balance_usd`), the same cell compares cached `extra.burn_samples` balance burn $/h with today account `actual_cost` implied $/h and shows a compact 对齐/偏离 cue. API-key balance is a cached extra snapshot written by `GET /admin/accounts/:id/usage` (`shouldRefreshUpstreamBalance`, 6 min, `force=true` bypasses TTL). When New API wallet credentials are set, that probe prefers `GET /api/user/self` then `GET /api/subscription/self` (`source=newapi_user_self` or `newapi_wallet_subscription`); the pair cell shows the combined remaining, and the pair PnL dialog splits wallet vs subscription. See [New API user wallet](#new-api-user-wallet). Opening or refreshing the pool page probes only stale/missing OpenAI/Anthropic API-key members (concurrency 4) and shows `upstream_balance_at`; the balance refresh icon forces a probe. Lite list does not probe. Display-only; stored billing is unchanged. Column settings live in the full-width table toolbar (`smart-schedule-pool-*` keys), not the right add/filter card. UsersView and the smart-schedule header add a `schedule_pnl` column after `smart_schedule` (pool totals of enabled-platform pairs only). Pair/user cells open `SchedulePnlTrendDialog` (default 24h hourly). Revenue is `SUM(actual_cost)` only on `true_cost IS NOT NULL` rows. AccountsView `AccountUsageCell` is unchanged. GET smart-schedule hydrates read-only member `priority` from live accounts; the pool column binds that live account field and never `sort_order` or a membership snapshot. Pool remove is “remove from pool”, not account delete. Pair-cap edits stay draft until Save; account concurrency / priority / upstream rate / schedulable save immediately via the account update API. Inline priority edits PATCH `accounts.priority` like the accounts page. Pool **移到顶部**, **排序（手动）**, and interval **自动排序** must never write `accounts.priority` or `extra.list_order`. They persist `user_smart_schedule_accounts.sort_order` immediately via `PATCH /admin/users/:id/smart-schedule/:platform/sort-order` (Save of caps is not required; refresh keeps the order). PUT Save also sends `sort_order` so a later replace-all does not wipe it. **移到顶部** assigns that member 1 and the rest 2..N (existing `sort_order` then id). **自动排序** reorders current-platform members by admission → this-user producing (`pair_current > 0`, binary) → `upstream_rate_multiplier` (lower first; type default when missing/negative) → pair headroom → account concurrency → existing `account.priority` (read-only tie-break) → `id`, then writes 1..N to `sort_order` only. It does not change membership, is not Save-gated, and does not use account-global `last_used`. A producing expensive account can still rank above an idle cheaper peer; rate only breaks ties inside the same producing bucket. Empty/null pair cap still means no extra cap (save semantics unchanged; `0` / null is not 999). The hot path still writes and releases Redis pair slots `concurrency:account_user:{accountID}:{userID}:{platform}` for every enabled closed-pool member request so occupancy can be counted; uncapped is count-only and never `pair_full`. Pair occupancy is **live in-flight only**: Get/Acquire drop members older than the pair live window (90s, refreshed while the request ctx is live) and members whose requestID is not this process prefix. Missing Release / SIGTERM leftovers must not keep showing hundreds of slots or block a real cap. `ClearAccountSlots` (account close/ops clear) also deletes `concurrency:account_user:{accountID}:*`. `GetAccountUserConcurrencyBatch` must keep `ScheduleLookupPlatform` on the detached Redis ctx so openai/antigravity hydrate does not read the `_` shard. Empty platform becomes `_`. Users not in a smart-schedule closed pool keep the old rule (pair slots only when a real cap N≥1). The pair badge always shows this user’s live occupancy: `current / max` when cap ≥ 1, or `current / 999` when uncapped (999 is display-only, never a real cap, never `account.concurrency`). While `probing`, the denominator is GET `probe_cap` (`min(desired, member cap)` or desired; desired is N成功率 when `probe_concurrency_mode=follow_n`, or `probe_concurrency` when custom) — never 999. Pair quality uses two last-N windows (`quality_min_ttft_samples` / `quality_min_success_samples`); compat `quality_window_n` echoes max. Launch: both N start equal to that user’s previous single N. This is not account-quality N or user-schedule N. GET also echoes `probe_concurrency_mode` / `probe_concurrency` on the platform policy. GET hydrates `current_concurrency` for all pool members from those pair keys, plus read-only `cooldown_until` from HASH `smart-schedule:cooldown:{platform}:{accountID}` field `u:{userID}`, plus `probing` + `probe_cap` from HASH `smart-schedule:probe:{platform}:{accountID}`, plus `pinned` from HASH `smart-schedule:pinned:{platform}:{accountID}`. Pair quality lives at `smart-schedule:pair-quality:{platform}:{accountID}` (HASH `u:{userID}`), with per-user trend/event keys `smart-schedule:pair-quality-trend:{platform}:{accountID}:{userID}` and `smart-schedule:pair-quality-events:{platform}:{accountID}:{userID}`. Missing probe/pin mark is not probing/pinned (no backfill). `default_platform` prefers an enabled platform, then the most recently updated / largest pool. The page opens that tab (zuoge85 lands on openai). The pool toolbar is three distinct regions: **add** and **in-pool filters** live in the right card (compact `w-fit` add bar: left = typeahead + filtered-add + one-click currently-scheduling presets; right = manual refresh + auto-refresh dropdown + 排序（手动） + interval auto-sort dropdown; filters use account-list chrome: search / type / schedulable / admission); **bulk** is full-width with the table under the cards (select page / select filtered / clear, batch remove-from-pool, batch/all pair cap — never account delete). The filtered-add dialog reuses `AccountTableFilters` (other tabs lock platform; AG tab defaults to all and can include OpenAI) plus schedulable / currently-scheduling / proxy, previews candidates not already in this platform pool, then add-selected or add-all-matching. The AG pool table also has a read-only Claude→GPT bridge column from `extra.openai_claude_gpt_bridge_enabled`. Membership edits stay Save-gated. Pool filters cover type, account schedulable, and admission (`selectable` / `probing` / `resumed` / `pinned` / `will_cool` / `cooling` / `paused` / `pair_full` / `stopped` / `unsaved_preview`). Live locks are `paused` (durable membership flag) and Redis pair cooldown (`cooling`). `pinned` is a long-lived manual exemption (not a lock). Cooldown wall-clock expiry enters `probing` (windows zeroed, `u:`/`w:` cleared, in-flight cap clamped), never `pinned`; leftover resume grace must not skip probe graduate. The UI does not invent probing from an expired `cooldown_until`. Graduate still looks at first-token as well as success: `ok_samples < N成功率` does not graduate; `ttft_samples < N首字` may still graduate; first-token full + p50 miss does not graduate. Do not change this to “success only”. Graduation reads pair windows, not the account/user last-N cell. A saved+enabled gate miss without a cooldown HASH field is `will_cool` (hint: next selected request will `StartCooldown`). An unsaved tighter form, or a disabled platform, is `unsaved_preview` — not a lock; Resume is disabled there. Successful 豁免期 (`resumed`) clears cooldown and writes `UserQualityResumeActive` grace; grace end is 调度 (keep windows), not 考察. `paused` has no automatic exit. `GET /admin/accounts/quality-stats/batch` does not currently return `resume_users`; the page keeps session-local grace after Resume and will also read those fields if present. Auto-sort ranks preview/hint with will-cool (still pickable), worse than selectable/resumed but better than pair-full / cooling / stopped. Batch remove/apply-cap run on the current filtered selection and stay Save-gated. Auto-refresh copies the account-list dropdown (5/10/15/30s), lives in the add-bar right cluster, spins the trigger refresh icon plus countdown while on, and skips overwriting dirty membership/caps. Interval auto-sort is a toggle next to **排序（手动）**. It does not start a second timer: it follows the auto-refresh interval, writes the same ranking silently after each auto-refresh finishes, persists enable under smart-schedule-auto-sort, and skips while a sort/refresh is in flight. Enabling it alone does nothing until auto-refresh is on; when both are on the sort button reuses the refresh countdown. Sorting is client-side; default and post auto-sort / 移到顶部 is `sort_order asc` (`smart-schedule-pool-sort`). A leftover stored `priority` sort is rewritten to `sort_order`. Column settings reuse the account-list visibility / order / width pattern, persisted under `smart-schedule-pool-*` keys so they do not share the account list layout. Select, name, and actions stay pinned. The hot path still uses `accounts.priority` unchanged.
...
| Input | Entry | Stored |
|-------|--------|--------|
| OAuth refresh token | 添加账号手动输入 RT / **更新凭证** 纯字符串 | `credentials.refresh_token`；校验时向上游换 AT |
| ChatGPT `/api/auth/session` or Codex session JSON | 添加账号 OpenAI OAuth 第二步粘贴；**更新凭证** 也可贴同一 JSON | JWT `accessToken` → `access_token`；`sessionToken` 写入 `credentials.chatgpt_session_token` 并用 `GET /api/auth/session` 换新 AT；**永不**写成 `refresh_token` |
| Codex PAT `at-*` | 添加账号 → Codex PAT | `auth_mode=personalAccessToken`；不走 OAuth refresh |

The account list toolbar no longer has「导入 Codex 会话」. Batch ingest still
uses `POST /admin/accounts/import/codex-session`; Add Account reuses that API
and sends step-1 `name` / `notes` / `group_ids` / `proxy_id` / concurrency /
priority / rates / `update_existing=true`.
...
kept. Both sides having a different `chatgpt_account_id` **or** a different
`chatgpt_user_id` is rejected; a missing id on the row is backfilled. PAT rows
reject session JSON (`OPENAI_SESSION_AUTH_MODE_MISMATCH`) and are skipped by
the import identity index. Default validate checks JWT expiry and does **not**
treat the JSON as an RT. `{ "refresh_token": "..." }` still uses the RT path.
JSON that also contains `refresh_token` / `tokens.refresh_token` (typical Codex
`auth.json`) identity-checks first, then hands the extracted RT to the same
OAuth/skip-validate path — a stale JWT must not block that handoff. JSON arrays
are rejected (single-account update, not bulk import). OpenAI account tests
take AT from `OpenAITokenProvider.GetAccessToken` (session/RT refresh) instead
of only reading the stored access token.
...
| extra (JSONB) | 同上 | 平台特有配置：allow_overages, mixed_scheduling, privacy_mode, model_rate_limits, `oauth_fleet_soft_429`（OAuth/setup-token 三态覆盖：true/false/缺省） |
...
| **Frontend Component** | `frontend/src/components/account/UnbindSubscriptionGroupsDialog.vue` | 按计费倍率解除订阅分组（预览 / 确认写入） |
...
列表展示序是 `extra.list_order`（越大越靠上）。**移到顶部**和页内拖拽 `PUT /admin/accounts/reorder` 同时写 `extra.list_pinned=true`。规则排序（手动 / 跟随自动刷新）走 `PUT /admin/accounts/reorder-auto-sort`：只改当前筛选里未手置顶的号，写入保留带 `N…1`，禁止 now 毫秒重铺，不改请求外账号、不写 `accounts.priority`、不改公共调度热路径。前端比较器复用 `comparePoolAutoSort`，同档先按 `fallback_only`（非 fallback 靠前）。已符合则不写库。
...
```
CreateAccountModal.vue OpenAI OAuth step 2: paste ChatGPT session / Codex auth.json
  -> POST /api/v1/admin/accounts/import/codex-session
     (step-1 name/notes/group_ids/proxy_id/concurrency/priority/rates, update_existing=true)
  -> account_codex_import.go: ImportCodexSession()
        ...
        -> access-only Find: fingerprint first; if no hit, same-user row that already has RT
        -> AdminService.UpdateAccount/CreateAccount()
        -> update 后失效 token cache
```

账号页工具栏不再出现「导入 Codex 会话」。`CodexSessionImportModal.vue` 可保留为未接线组件；import API 不变。

重要机制：

- **完整会话身份优先级**：携带 `refresh_token` 时，`chatgpt_user_id` 是第一身份；共享 `chatgpt_account_id` 只在双方 user id 不冲突时作为兼容回退，支持存量缺失 user id 的账号回填。
- **access-only 隔离**：不携带 `refresh_token` 时 Find 仍以 access-token 指纹为先。两个都无 RT、指纹不同的短命 AT 即使同一 user 也不得合并。若已有带 RT 的同 user 行，access-only / session 导入更新该行并保留 RT，不 `CreateAccount`。
- **凭据保护**：access-only 更新已有完整 OAuth 账号时保留 `refresh_token`、`client_id`、`id_token` 和账号到期/自动暂停设置，避免用短期会话降级可自动续期账号。incoming 无 `id_token` 时不得 `delete` 已有值。ChatGPT `sessionToken` 单独存在 `chatgpt_session_token`，由 token refresher 按约 25 分钟换新 AT；JWT `exp` 不能当作 ChatGPT 会话仍有效。无 ST/RT 的 AT 在导入时会打 `accounts/check`，ChatGPT 401 则拒绝入库。
...
| 按计费倍率解除订阅分组 | 账号列表工具栏预览/确认：`BillingRateMultiplier() >` 阈值的账号按账号减去订阅分组，`BindGroups` 写入剩余 ID。不走批量 `group_ids`，不改调度开关 | `admin_unbind_subscription_groups.go`, `UnbindSubscriptionGroupsDialog.vue` |
...
| 账号质量列（首字/成功率） | 账号格与用户格都是 last-N：账号格人口是该号全部用户 \(Q_a\)（全站 `account_quality_window_n`，默认 20）；用户格人口是该用户全部账号 \(Q_u\)（`users.quality_window_n` 覆盖，NULL 继承全站 N）。成功与首字来自完成挂钩（usage + 计入的 ops 错误）。账号维同时算两套错误：`terminal_error_*` 与 `failover_error_*`。`ErrorCount` / 调度成功率默认用 terminal；`schedule_use_failover_error_rate=true` 时才拷 failover。桥接另计 `bridge_*`，只在弹窗展示。**账号单元格**与**用户单元格**都画 p50 + 展示用 p95 + 调度成功率 + 含 failover 对照 + k/N。空窗口 JSON 的 rate 必须是 `null`。账号列表走 `POST /admin/accounts/quality-stats/batch`；用户列表/页头走 `POST /admin/users/quality-stats/batch`（last-N，**禁止**再把 15 分钟 SQL 当列表真相，也禁止再钉 `failover_error_count=0`）。列隐藏时不请求；30s 缓存。曲线默认隐藏 p95 | `account_quality.go`, `account_quality_last_n.go`, `user_quality_last_n_cache.go`, `AccountQualityCell.vue`, `AccountStabilityDialog.vue`, `UserQualityDialog.vue` |
| 账号质量历史快照 | 维护任务每 5 分钟把 live last-N \(Q_a\) 落进 `account_quality_snapshots`、\(Q_u\) 落进 `user_quality_snapshots`（有样本才写，保留 7 天，多实例 leader lock）。`GET /admin/accounts/:id/quality-history` 与 `GET /admin/users/:id/quality-history` 默认最近 24 小时、最大 7 天。硬关闭只评估账号 live last-N，不读快照行。 | `account_quality_maintenance.go`, `account_quality_snapshot_repo.go`, `user_quality_snapshot_repo.go`, `migrations/199_account_quality_snapshots.sql`, `migrations/209_user_quality_snapshots.sql` |
| 账号质量硬关闭 | 默认全关。全局 KV `quality_hard_close_settings` + 账号 `extra.quality_hard_close` 两层都开才评估本轮 live last-N \(Q_a\)；越界则 `SetTempUnschedulable(..., "quality_hard_close:...")`，不改 `schedulable`。同一 JSON 里的 `schedule_use_failover_error_rate`（默认 `false`）决定 live `ErrorCount` 用 terminal 还是 failover；不是 per-account overlay。已有未过期临时停调度则跳过（冷却一次，也不覆盖 529/401）。不进 public settings | `account_quality_hard_close.go`, `GET/PUT /admin/settings/quality-hard-close`, `GET/PUT /admin/accounts/:id/quality-hard-close` |
...
- **账号质量格与硬关闭是 last-N**：`POST /admin/accounts/quality-stats/batch` 与 `EvaluateAccountQualityHardClose` 读同一份 \(Q_a\)（全站 N，默认 20；未满 N 不判断该指标）。完成入窗即重算。5 分钟 tick 只快照历史，禁止再用 15 分钟 SQL `Replace` live。用户维 `users/quality-stats/batch` 读 \(Q_u\)（`user-quality:last-n:{userID}`，该用户全部账号；N 来自 `users.quality_window_n` 或继承全站）。`account_quality_window_n` 不是智能调度 `quality_window_n`。
- **质量快照是当时 live 的采样**：账号点是 `captured_at` 时的 last-N \(Q_a\)；用户点是同一时刻的 \(Q_u\)（`user_quality_snapshots`）。无样本不写空行。调度 `ErrorCount` 必须排除 Claude→GPT 桥接失败以及客户端/路由的模型不存在；桥接走 `bridge_*`。用户列表 last-N 与账号格同一套计入挂钩，不再把 15 分钟 SQL / `failover_error_count=0` 当列表真相。残留的 `GetUserQualityStatsBatch` SQL 若仍被其它调用方使用，FILTER 必须写 `WHERE FALSE` 不能写 `WHERE 0`。账号维始终 FILTER 两套错误率；默认 `schedule_use_failover_error_rate=false`。打开开关后 `ApplyAccountQualityScheduleCaliber` 才把 failover 拷进 `ErrorCount`。账号/用户 batch cache key 都带 `failover:`。不要手写 rebuild / 回填 snapshot。
- **质量硬关闭默认全关**：部署后不会自动停号。评估用 last-N live \(Q_a\)（入窗后或 tick 兜底），不是快照行或 15 分钟 SQL；账号 overlay 只允许 `UpdateExtra` 合并 `quality_hard_close`，不要用 `Update()` 整表替换 Extra。overlay 的样本/N 不单独改窗。
...
- **ChatGPT session JWT `exp` 不是 liveness**：粘贴的 `accessToken` 即使 `exp` 仍在未来，ChatGPT 也可 401 `token_expired`。有 `chatgpt_session_token`、无 RT 的号，请求前必须 `GET https://chatgpt.com/api/auth/session` 换新 AT，再用 `accounts/check` 确认。check 返回 `token_expired` 时不得打 `session_refreshed_at` / `chatgpt_session_verified`，也不得把该 JWT 发给 Codex。用户需在浏览器重新打开 chatgpt.com 后粘贴最新 session JSON。换票失败（含 Cloudflare 403 HTML → `OPENAI_SESSION_REFRESH_CF_BLOCKED`）必须把错误返回给调用方，禁止 `UseExistingToken` 把库存 JWT 发出去。
