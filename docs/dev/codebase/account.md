...
| OpenAI OAuth 7d L$ | 仅 `platform=openai` + OAuth。7d 条旁 `L$` 是当前 LiteLLM 标价 × 真实 token，对齐该号 `codex_7d_reset_at`；不改 A$/U$/`actual_cost`/`true_cost`/展示变换。无 Codex 7d 快照不编造窗口。`reset_at` 前跳 ≥1h 时把上期 L$+used% 写入 `openai_oauth_7d_cycles`，列表只展示最近一期 | `openai_7d_litellm.go`, `openai_7d_cycle_repo.go`, `UsageProgressBar.vue`, `migrations/220_openai_oauth_7d_cycles.sql` |
