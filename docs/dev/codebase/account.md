...
Unpooled users (`lookupEnabledSmartPolicy(..., SmartScheduleLookupPlatform(account, hint, bundle)) == nil`, including `userID<=0` fail-open): if a cheaper schedulable peer has `LoadRate < 100`, **session** sticky on a higher-rate account is cleared and the cheap peer is selected. If the cheap tier is full, the pin stays and that account is used immediately (no extra cheap-tier wait). OpenAI advanced scheduling also acquires a higher-rate account that still has headroom instead of returning a cheap-tier WaitPlan. Pooled users (EnabledPolicy on the **lookup platform**, never a cross-pool fallback) keep today's pin and may still WaitPlan on the cheap tier. `previous_response` sticky is never cleared for a cheaper peer. See [gateway.md](./gateway.md#upstream-rate-overlay).
