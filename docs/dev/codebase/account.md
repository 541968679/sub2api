...
| 账号质量列（首字/成功率） | 账号格与用户格都是站点级 last-N（`account_quality_window_n`，默认 20）：账号格人口是该号全部用户 \(Q_a\)，用户格人口是该用户全部账号 \(Q_u\)。成功与首字来自完成挂钩（usage + 计入的 ops 错误）。账号维同时算两套错误：`terminal_error_*` 与 `failover_error_*`。`ErrorCount` / 调度成功率默认用 terminal；`schedule_use_failover_error_rate=true` 时才拷 failover。桥接另计 `bridge_*`，只在弹窗展示。**单元格**画 p50 + 调度成功率 + 含 failover 对照 + k/N。空窗口 JSON 的 rate 必须是 `null`。账号列表走 `POST /admin/accounts/quality-stats/batch`；用户列表/页头走 `POST /admin/users/quality-stats/batch`（last-N，**禁止**再把 15 分钟 SQL 当列表真相，也禁止再钉 `failover_error_count=0`）。列隐藏时不请求；30s 缓存。曲线默认隐藏 p95 | `account_quality.go`, `account_quality_last_n.go`, `user_quality_last_n_cache.go`, `AccountQualityCell.vue`, `AccountStabilityDialog.vue`, `UserQualityDialog.vue` |
...
- **账号质量格与硬关闭是 last-N**：`POST /admin/accounts/quality-stats/batch` 与 `EvaluateAccountQualityHardClose` 读同一份 \(Q_a\)（全站 N，默认 20；未满 N 不判断该指标）。完成入窗即重算。5 分钟 tick 只快照历史，禁止再用 15 分钟 SQL `Replace` live。用户维 `users/quality-stats/batch` 读 \(Q_u\)（`user-quality:last-n:{userID}`，同一 N，该用户全部账号）。`account_quality_window_n` 不是智能调度 `quality_window_n`。
