...
| 账号质量列（首字/成功率） | 列表默认显示最近 **15 分钟**滚动窗口的平均 TTFT 与成功率；成功与首字来自 `usage_logs`，失败来自 `ops_error_logs`（status≥400，排除 count_tokens）。**单元格**只在 `success_count > 0` 且样本达到 min 时画百分比；空窗口或仅有失败、无成功 usage 显示 `—`，避免刚开启调度被画成 0%。JSON 空窗口的 `success_rate` / `error_rate` 必须是 `null`，不能是 `0`。门控仍用 success+error（纯失败窗口 success_rate=0）。账号列表与智能调度池表都走账号维 `POST /admin/accounts/quality-stats/batch`；用户列表/页头走用户维 `POST /admin/users/quality-stats/batch`。列隐藏时不请求；30s 缓存。不参与调度/熔断。账号管理页点击单元格打开 `AccountStabilityDialog`（24h 曲线 + 账号硬关闭表单）；曲线默认隐藏 p95，左轴按当前可见 TTFT 序列缩放（显示 p95 时若会压扁 p50 则按 `max(p50)*2.5` 裁剪）；用户页只读 | `account_quality.go`, `usage_log_repo.go`, `accountQualityStats.ts`, `AccountsView.vue`, `AccountQualityCell.vue`, `AccountStabilityDialog.vue` |
