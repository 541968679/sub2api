...
使用记录 API 返回前的处理流程 (usage_handler.go:149-161):
  ...
  原始 UsageLog DTO
    ...
    │   影响字段: InputTokens, OutputTokens 及对应 Cost；CacheReadCost 可按展示价重算
    │   不影响: CacheReadTokens, CacheCreationTokens, RateMultiplier, ActualCost
...
`ApplyUserDisplayRate` and downstream response `RateScale` may scale
`cache_read_cost` so `total_cost * rate_multiplier == actual_cost` remains true,
but they do not multiply `cache_read_tokens` / `cache_read_input_tokens`.
...
背景:官方缓存写入分两档(5m=1.25×输入价,1h=2×输入价)。走 LiteLLM 源价的模型
(claude-sonnet-5/claude-fable-5)本就按官方分档正确计费——生产 sonnet-5 纯 1h 行隐含
$4.0/MTok = 官方优惠期 1h 价(2×$2),经官方价目表核实无误。**被压平的是配了全局覆盖的
模型**(opus 系列 cache_write=$10 平价、sonnet-4-6=$5 平价):单一 `cache_write_price`
同写三档,1h 溢价无法表达——本字段即为此而设。注意 sonnet-5 优惠价 2026-08-31 结束后
LiteLLM 价格将回到 $3/$15 档(5m $3.75 / 1h $6.00)。
LiteLLM 数据侧:`enableBreakdown = price1h > 0 && price1h > price5m`(GetModelPricing),
源数据缺 1h 价的模型必须靠该覆盖字段表达溢价。

**分档口径 FAQ**:① 所有支持缓存的 Claude 模型都有 5m/1h 两档,是否出现取决于调用方请求的
`cache_control.ttl`(1h 需 beta 头);② 无分档价的模型(SupportsCacheBreakdown=false)走
平价回退 `total × CacheCreationPricePerToken`;③ 上游未返回 ephemeral 5m/1h 细分时,
全部按 5m 档价计费(computeCacheCreationCost 显式回退),展示侧同样退化为 5m 档倍率。

**5m/1h 配置面补全(2026-07-02 第三批,migration 173)**:
- `user_model_pricing_overrides.cache_write_1h_price`:用户级真实 1h 价,
  `applyUserModelPricingOverride` 与全局同语义(单独写 1h 档 + 强制 SupportsCacheBreakdown)。
- `display_cache_creation_1h_price`(global + user):1h 档展示价,NULL 回退 5m 档展示价
  (`display_cache_creation_price`)。usage-log 侧(ApplyDisplayTransform):行有细分且两档
  展示价不同时,按真实档价比例(DisplayPricingConfig.RealCacheWritePrice/RealCacheWrite1hPrice,
  未知按 1:1)拆分落库成本、各档独立反算——展示成本按构造守恒;只配 5m 档展示价时保持
  "总成本反算"路径不变。下游侧 `CacheCreate1hMult` 分母改用 1h 展示价。
- LiteLLM 管理载荷(`LiteLLMPrices`)新增 `cache_write_1h_price`
  (源字段 `cache_creation_input_token_cost_above_1hr`),两个定价表单的参考区/建议值/占位符可见。
