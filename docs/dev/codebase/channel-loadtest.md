...
- Excel export: `GET|POST /api/v1/admin/channel-loadtest/runs/:id/export` (finished runs only). Filename `{startLocalTime}_{models}.xlsx`. POST body can omit sheets (`include_*`) and 测试条件 fields (`condition_fields`); Admin opens a default-all-selected dialog first. Workbook sheets: 测试条件, 总览, 首字延迟(ms), 总耗时(ms), 成功率(%), with unit columns/headers. No SLA PASS/FAIL. Built by `loadtest.BuildExcelReport` (excelize).
