...
| 模型映射 | [model-mapping.md](model-mapping.md) | 模型白名单/映射配置、默认映射、四平台可编辑 `/v1/models` 目录、网关解析、通配符、Claude-GPT 账号级映射 | 2026-09-05 |
...
| Ops | [ops.md](ops.md) | Admin operations dashboard, lightweight realtime projections, batched group capacity, Redis slot cleanup, availability alerting, Recovered list visibility (`include_recovered`), user/compare/schedule error-rate badges, schedule error whitelist custom/from-log, and admin dual-sided upstream/downstream error display；生产 runbook 在 [../PRODUCTION.md](../PRODUCTION.md) | 2026-08-23 |
