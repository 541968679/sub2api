...
| 智能调度 | [scheduler.md](scheduler.md) | 选号热路径、硬门槛、粘性、snapshot、智能调度准入；改选号先读此手册 | 2026-08-28 |
| 账号管理 | [account.md](account.md) | 账号 CRUD、Antigravity/Gemini OAuth、AI Credits、批量导入、OpenAI Claude-GPT bridge 账号绑定、账号级用户调度 allow/deny、账号/用户 last-N 质量格、质量硬关闭、账号错误率双口径与调度开关、用户智能调度账号池、账号管理列表手动/自动排序（`list_order` / `list_pinned`，不改热路径）。选号策略见 [scheduler.md](scheduler.md) | 2026-08-30 |
| 模型映射 | [model-mapping.md](model-mapping.md) | 模型白名单/映射配置、默认映射、四平台可编辑 `/v1/models` 目录、网关解析、通配符、Claude-GPT 账号级映射、模型配置定价列表国内模型种子、CCS 导入 picker 列表、GPT-6 Sol/Luna 与 Claude Opus 5.5 | 2026-09-23 |
| 计费系统 | [billing.md](billing.md) | 五级定价链、费用计算、展示变换、费率乘数、缓存命中计费、Claude-GPT bridge 缓存展示 | 2026-06-03 |
| API 网关 | [gateway.md](gateway.md) | 请求转发、负载均衡、熔断、SSE 流式、账号级用户调度过滤、Antigravity Claude-GPT bridge preflight、Codex remote compact V2 API-key 兜底、CC→Responses 同步缓冲超时与 API Key 非流式 JSON、可选 NewAPI slim completed（默认关）；raw CC 空 `model` 补请求模型；选号策略见 [scheduler.md](scheduler.md)；bridge timeout 调查见 [../OPENAI_CLAUDE_GPT_BRIDGE_TIMEOUT_INVESTIGATION_2026-07-10.md](../OPENAI_CLAUDE_GPT_BRIDGE_TIMEOUT_INVESTIGATION_2026-07-10.md)；Grok/Codex 多轮与 OpenAI Key 跨平台 Grok 见 [../GROK_CODEX_AND_CROSS_PLATFORM_RESEARCH_2026-07-15.md](../GROK_CODEX_AND_CROSS_PLATFORM_RESEARCH_2026-07-15.md)；等头/首有效帧超时标记只进 Ops，不回放给下游 | 2026-09-20 |
...
| Ops | [ops.md](ops.md) | Admin operations dashboard, lightweight realtime projections, batched group capacity, Redis slot cleanup, availability alerting, Recovered list visibility (`include_recovered`), user/compare/schedule error-rate badges, schedule error whitelist custom/from-log, and admin dual-sided upstream/downstream error display；生产 runbook 在 [../PRODUCTION.md](../PRODUCTION.md) | 2026-09-07 |
