...
| 账号管理 | [account.md](account.md) | 账号 CRUD、Antigravity/Gemini OAuth、AI Credits、批量导入、OpenAI Claude-GPT bridge 账号绑定、账号级用户调度 allow/deny、账号/用户 last-N 质量格、质量硬关闭、账号错误率双口径与调度开关、用户智能调度账号池 | 2026-08-21 |
...
| Ops | [ops.md](ops.md) | Admin operations dashboard, lightweight realtime projections, batched group capacity, Redis slot cleanup, availability alerting, Recovered list visibility (`include_recovered`), and user/compare/schedule error-rate badges；生产 runbook 在 [../PRODUCTION.md](../PRODUCTION.md) | 2026-08-18 |
