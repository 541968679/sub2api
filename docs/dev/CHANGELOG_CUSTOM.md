...
- Removed a stray local type declaration from Antigravity schema cleaning, added precise unused suppressions for retained helper/request types across bridge, image accounting, and pricing code, documented an intentional `time.Time` location-identity comparison in the credit snapshot test, formatted the pricing repository/handler files, and updated the stale usage stats SQL mock column list.
