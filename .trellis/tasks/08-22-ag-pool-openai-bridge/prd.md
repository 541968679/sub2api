...
openai 池与 antigravity 池 **完全独立**：同一 OpenAI 账号可以同时在两池，但占用、冷却、probe、pin、pair-quality、pair cap、ObservePairCompletion 都按 **(user, account, platform)** 隔离。AG 池冷却不得让 openai 池同号一起冷却，反之亦然。不要回落混用。

## User value

生产用户 12（`gybilly2023@gmail.com`）分组 15 `antigravity billy` 几乎全是桥接，但只有 openai 策略、没有 AG 策略。桥接热路径按 `account.Platform=openai` 查闭池，于是分组 15 与分组 19 原生 GPT 共用同一 openai 闭池。修完后：AG 池可加 OAI 号；桥接/AG 分组 lookup 用 AG 池（决策 C）；两池 Redis 状态互不影响。
...
- R4. 桥接 / Antigravity 分组请求的智能调度策略查找必须用 **antigravity 池**。Lookup、准入、pair 槽、未入池判定、ObservePairCompletion 必须同一 helper。AG 策略 nil/空池/disabled 时 fail-open 到账号侧 allow/deny，**禁止**回落 openai 池。
...
- R11. openai 池与 AG 池 Redis 状态完全独立。占用 / 冷却 / probe / pin / pair-quality / pair cap 按 `(user, account, platform)` 隔离。不要回落混用。
...
- [x] AC8. 单测覆盖：sanitize 允许 OAI→AG；sanitize 仍拒 OAI→anthropic；桥关号在池但不被桥接选中；lookup 按决策 C；双池 persist；两池冷却/占用隔离；AG 策略 disabled 时 fail-open 账号侧。
...
| **C** | 热路径策略平台 | **桥接 / AG 分组查 antigravity** | AG 策略 nil/空池/disabled：fail-open 账号侧 allow/deny，**禁止**回落 openai 池。 |
