...
PUT body: `{enabled, quality_*, quality_min_ttft_samples, quality_min_success_samples, quality_window_samples|quality_window_n, probe_concurrency_mode, probe_concurrency, cooldown_minutes, accounts:[{account_id, max_concurrency?}]}`. Copy copies enabled/thresholds/both N/cooldown/probe settings only, never members or caps; do not echo the two N into one value. Do not copy account-quality N into `probe_concurrency`. GET shows `enabled=false` when the stored row is enabled but the pool is empty. GET echoes the two real N columns; `quality_window_samples` / `quality_window_n` are compat aliases = `max(N首字, N成功率)`. Also echo `probe_concurrency_mode` / `probe_concurrency` (omit/empty mode → `follow_n`). Launch: both stored N already equal that user’s previous single N — do not default N首字 smaller.
