...
- Redis soft cooldown: `smart-schedule:soft-cool:{platform}:{accountID}` HASH `u:{userID}` = the same FIFO JSON shape as pair quality. One window per cooling pair: same-platform pool samples **after that pair’s cooldown write**, **exclude self**. Do not reuse \(Q_u\) or other pairs’ \(Q_{a,u}\). `StartCooldown` zeros the field only when `HSETNX` succeeds; `SetCooldown` always zeros. Leaving cooldown (`ClearCooldown`, wall-clock expire, or soft early exit) deletes the field. TTL only lengthens (`cooldown_minutes` + buffer). Hard policy must not ingest this key.
- Redis pair quality: `smart-schedule:pair-quality:{platform}:{accountID}` HASH `u:{userID}` = two FIFO windows (`W_ttft`, `W_ok`). Trend list `smart-schedule:pair-quality-trend:{platform}:{accountID}:{userID}` (TTL 24h). Event list `smart-schedule:pair-quality-events:{platform}:{accountID}:{userID}` (TTL 7d). Event `soft_cooldown_end` is soft early-exit only; do **not** reuse `cooldown_end`. Wall-clock still uses `expiry_zero` (when probe v2 is off) then `probe_enter`.
...
PUT body: `{enabled, quality_*, quality_min_ttft_samples, quality_min_success_samples, quality_window_samples|quality_window_n, probe_concurrency_mode, probe_concurrency, cooldown_minutes, soft_cooldown, accounts:[{account_id, max_concurrency?}]}`. Omitted `soft_cooldown` = false (hard). Copy copies enabled/thresholds/both N/cooldown/`soft_cooldown`/probe settings only, never members or caps; do not echo the two N into one value. Do not copy account-quality N into `probe_concurrency`. Site quality-template schema must **not** include `soft_cooldown`. GET shows `enabled=false` when the stored row is enabled but the pool is empty. GET echoes the two real N columns; `quality_window_samples` / `quality_window_n` are compat aliases = `max(N首字, N成功率)`. Also echo `probe_concurrency_mode` / `probe_concurrency` (omit/empty mode → `follow_n`) and `soft_cooldown`. GET hydrates member `soft_cooldown_progress` **only** when the platform is soft **and** that member is cooling. Launch: both stored N already equal that user’s previous single N — do not default N首字 smaller.
...
`probing` (考察) is entered by **cooldown wall-clock expiry**, **soft early-exit** (`soft_cooldown_end`), or by admin (including 调度→考察). Enter: `HDEL` cooldown, **zero both windows**, `ClearPairResume` (no pair `u:`/`w:`), `HSET` probe mark. In-flight pair cap = `min(desired, member cap)` or desired if unset. `desired` is **N成功率** when `probe_concurrency_mode=follow_n` (omit/empty), or `probe_concurrency` (1–100) when `custom`. Member `max_concurrency` is a hard ceiling. This is **not** account-quality global N. Invalid custom (0, >100, custom without a number) → `SMART_SCHEDULE_INVALID_QUALITY`; do not silently fall back. During probing, ingest and evaluate `Q_{a,u}`. Graduate → 调度: **keep** windows, `HDEL` probe mark, lift to member cap. Probe cool uses the same or/and as pair cooldown (unfilled metrics do not participate) plus a **probe-only** override: `and` + both windows full (each vs its own N) + one pass one fail → `StartCooldown` (anti-deadlock). That override must **not** change `and` for selectable. Graduate requires `W_ok` full N成功率; configured success-rate threshold must pass; `ttft_samples < N首字` does **not** block; if `W_ttft` is full (N首字) and p50 is configured, p50 must pass. No time-based graduate. No traffic → stay probing.

`cooling` clears `paused` and the probe mark, `HSET`s cooldown to `now+policy.cooldown_minutes` and deletes `u:`/`w:`. Cooling/paused pairs do not ingest (not into \(Q_{a,u}\) and not as soft-window producers). Auto cooldown expiry enters **考察**, not 调度: `HDEL` + zero windows + write probe mark + `ClearUserResume`, no time grace. Soft policy may **also** leave cooling early: after a non-cooling completion (including pin), ingest that sample into other same-user-same-platform cooling peers’ soft windows and evaluate `softCooldownMeets`. Meet = every **configured** selectable gate is **full and passing**. Success is one gate. TTFT and duration are **one** latency gate: every configured latency window must be full, then `pairQualitySelectableLatencyBlocked` must not fire. `or`/`and` combine success vs latency; do **not** OR a passing TTFT window against a broken duration window. Underfull ≠ pass. Do **not** treat `pairQualityBlocks == false` (fail-open) as pass. On meet: same enter-probe side effects as wall-clock expire, event type `soft_cooldown_end` then `probe_enter`. Wall-clock `cooldown_minutes` remains the ceiling. The request that cools account A must not enter A’s new soft window. Hard policy must not ingest soft windows. Pinned leftover-cooldown peers are not soft-window targets.
...
- Good: platform `soft_cooldown=true`, pair A cooling, pair B (same user/platform, not cooling) completes with a full passing soft window → A enters probe via `soft_cooldown_end`; wall-clock minutes still bound the wait if the window never meets.
...
## Common Mistake: soft cooldown treats fail-open as a meet

**Symptom**: a cooling pair graduates to probe after one or two peer samples, or after a cache miss, even though N is not full.

**Cause**: `softCooldownMeets` reused `pairQualityBlocks` / `!pairQualityBlocks` (underfull fail-open) or ingested the request that just cooled account A into A’s new soft window.

**Prevention**: configured gates meet only when full **and** the selectable breaker does not fire. TTFT and duration share one latency gate (`pairQualitySelectableLatencyBlocked` after every configured window is full). Underfull waits the wall-clock. Soft windows are per cooling pair, exclude self, skip pinned leftovers, and start after that pair’s cooldown write. Hard policy never writes `smart-schedule:soft-cool`. Event is `soft_cooldown_end`, not `cooldown_end`.
