...
- [ ] If this touches user smart-schedule: `admitsScheduleUser`, pair acquire, unpooled cheaper-tier, and `ObservePairCompletion` all use `SmartScheduleLookupPlatform` — never `account.Platform` alone on bridge / AG-group OpenAI traffic. See `.trellis/spec/backend/account-user-schedule.md`.
