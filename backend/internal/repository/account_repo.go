...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if search != "" {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			s.Where(entsql.Or(
				entsql.ContainsFold(s.C(dbaccount.FieldName), search),
				sqljson.StringContains(dbaccount.FieldCredentials, strings.ToLower(search), sqljson.Path("email")),
				sqljson.StringContains(dbaccount.FieldExtra, strings.ToLower(search), sqljson.Path("email_address")),
			))
		}))
	}
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountCapacityRow, error) {
	...
	for rows.Next() {
		...
		var extraRaw string
		if err := rows.Scan(&row.GroupID, &row.AccountID, &row.Concurrency, &extraRaw, &row.SessionWindowStart, &row.SessionWindowEnd, &row.SessionWindowStatus); err != nil {
			return nil, err
		}
		if extraRaw != "" && extraRaw != "null" {
			if err := json.Unmarshal([]byte(extraRaw), &row.Extra); err != nil {
				return nil, err
			}
		}
		out = append(out, row)
