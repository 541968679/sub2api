...
WITH stats AS (
  SELECT
    ...
    ROUND(AVG(COALESCE(ul.true_first_token_ms, ul.first_token_ms))::numeric, 2)::float8 AS avg_first_token_ms,
    ...
    COUNT(CASE WHEN COALESCE(ul.true_first_token_ms, ul.first_token_ms) IS NOT NULL THEN 1 END)::bigint AS requests_with_first_token
  FROM usage_logs ul
  ` + join + `
  ` + where + `
  GROUP BY ul.model
