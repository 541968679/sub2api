...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/errors/stats", h.Admin.Ops.GetErrorLogStats) // before :id
...
func registerDashboardRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	dashboard := admin.Group("/dashboard")
	{
		...
		dashboard.GET("/cache-status", h.Admin.Dashboard.GetCacheStatus)
		...
		dashboard.POST("/users-burn-rate", h.Admin.Dashboard.GetBatchUsersBurnRate)
		dashboard.POST("/api-keys-usage", h.Admin.Dashboard.GetBatchAPIKeysUsage)
		dashboard.GET("/user-breakdown", h.Admin.Dashboard.GetUserBreakdown)
		dashboard.POST("/aggregation/backfill", h.Admin.Dashboard.BackfillAggregation)
		// 成本分析：包月/日限订阅成本利润统计
		dashboard.GET("/subscription-profit", h.Admin.Dashboard.GetSubscriptionProfit)
	}
}
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/smart-schedule/pnl/summaries", h.Admin.User.GetBatchSmartSchedulePnlSummaries)
		users.GET("/:id/quality-history", h.Admin.User.GetQualityHistory)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/refresh-token", h.Admin.Account.UpdateRefreshToken)
		accounts.POST("/:id/set-privacy", h.Admin.Account.SetPrivacy)
		accounts.POST("/:id/refresh-tier", h.Admin.Account.RefreshTier)
		accounts.GET("/:id/stats", h.Admin.Account.GetStats)
		accounts.POST("/:id/clear-error", h.Admin.Account.ClearError)
		accounts.GET("/:id/usage", h.Admin.Account.GetUsage)
		accounts.GET("/:id/openai-7d-cycles", h.Admin.Account.GetOpenAI7dCycleHistory)
		accounts.GET("/:id/today-stats", h.Admin.Account.GetTodayStats)
		accounts.POST("/today-stats/batch", h.Admin.Account.GetBatchTodayStats)
		accounts.POST("/quality-stats/batch", h.Admin.Account.GetBatchQualityStats)
		accounts.POST("/public-schedule-quality/batch", h.Admin.Account.GetBatchPublicScheduleQuality)
		accounts.GET("/:id/public-schedule-quality", h.Admin.Account.GetPublicScheduleQuality)
		accounts.POST("/:id/public-schedule-quality/state", h.Admin.Account.SetPublicScheduleQualityState)
		...
		accounts.POST("/:id/public-schedulable", h.Admin.Account.SetPublicSchedulable)
		accounts.POST("/:id/clear-stuck-runtime", h.Admin.Account.ClearStuckRuntime)
		...
		accounts.POST("/unbind-subscription-groups-by-rate", h.Admin.Account.UnbindSubscriptionGroupsByRate)
		accounts.POST("/openai-capability-reprobe", h.Admin.Account.OpenAICapabilityReprobe)
...
func registerGeminiOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	gemini := admin.Group("/gemini")
	{
		...
		gemini.POST("/oauth/refresh-token", h.Admin.GeminiOAuth.RefreshToken)
...
func registerGrokOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	grok := admin.Group("/grok-oauth")
	{
		...
		grok.POST("/refresh-token", h.Admin.GrokOAuth.RefreshToken)
		grok.POST("/create-account", h.Admin.GrokOAuth.CreateAccountFromOAuth)
		grok.POST("/accounts/:id/refresh", h.Admin.GrokOAuth.RefreshAccountToken)
		grok.POST("/accounts/:id/quota/query", h.Admin.GrokOAuth.QueryQuota)
		grok.POST("/accounts/:id/quota/reset", h.Admin.GrokOAuth.ResetQuota)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.GET("/public-schedule-quality", h.Admin.Setting.GetPublicScheduleQualitySettings)
		adminSettings.PUT("/public-schedule-quality", h.Admin.Setting.UpdatePublicScheduleQualitySettings)
		...
		adminSettings.GET("/openai-wait-timeout", h.Admin.Setting.GetOpenAIWaitTimeoutSettings)
		adminSettings.PUT("/openai-wait-timeout", h.Admin.Setting.UpdateOpenAIWaitTimeoutSettings)
		// 请求整流器配置
...
func registerSubscriptionRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	subscriptions := admin.Group("/subscriptions")
	{
		...
		subscriptions.POST("/:id/adjust-quota", h.Admin.Subscription.AdjustQuota)
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	usage := admin.Group("/usage")
	{
		...
		usage.GET("/stats/antigravity/curve", h.Admin.Usage.StatsAntigravityCurve)
		...
		usage.GET("/:id/user-view", h.Admin.Usage.GetUserViewPreview)
	}
}
...
func registerChannelLoadtestRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	lt := admin.Group("/channel-loadtest")
	{
		...
		lt.POST("/runs/:id/export", h.Admin.ChannelLoadtest.Export)
...
func registerImageChannelMonitorRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	monitors := admin.Group("/image-channel-monitors")
	{
		...
		monitors.POST("/:id/manual-test", h.Admin.ImageChannelMonitor.ManualTest)
		monitors.POST("/:id/manual-test/client-runs/:clientRunID/cancel", h.Admin.ImageChannelMonitor.CancelManualTestByClientRunID)
		monitors.GET("/:id/manual-test/:runID", h.Admin.ImageChannelMonitor.ManualTestStatus)
		monitors.GET("/:id/manual-test/:runID/images/:index", h.Admin.ImageChannelMonitor.ManualTestImage)
		monitors.POST("/:id/manual-test/:runID/cancel", h.Admin.ImageChannelMonitor.CancelManualTest)
