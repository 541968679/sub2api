...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		if !selection.Acquired {
			...
			if err != nil {
				reqLog.Warn("gateway.responses.pair_slot_acquire_failed", zap.Int64("account_id", account.ID), zap.Error(err))
				h.handleConcurrencyError(c, err, "account", streamStarted)
				return
			}
			if pairFull {
				reqLog.Info("gateway.responses.pair_full_after_wait", zap.Int64("account_id", account.ID))
