...
func TestOpenAIAnthropicClientFailoverErrorStopsAccountSwitching(t *testing.T) {
	h := &OpenAIGatewayHandler{}

	require.True(t, h.isAnthropicClientFailoverError(&service.UpstreamFailoverError{
		StatusCode:   http.StatusBadRequest,
		ResponseBody: []byte(`{"error":{"type":"invalid_request_error","message":"context window exceeded"}}`),
	}))
	require.False(t, h.isAnthropicClientFailoverError(&service.UpstreamFailoverError{
		StatusCode:   http.StatusBadGateway,
		ResponseBody: []byte(`{"error":{"type":"upstream_error","message":"temporary failure"}}`),
	}))
}
