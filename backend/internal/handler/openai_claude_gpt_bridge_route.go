...
func (h *OpenAIGatewayHandler) ClaudeGPTBridgeRoute(c *gin.Context) ClaudeGPTBridgeRouteAction {
	...
	logClaudeGPTBridgeRouteDecision(reqLog, decision, reqModel, "preflight", routeStart)
...
func (h *OpenAIGatewayHandler) respondClaudeGPTBridgeSelectionRace(c *gin.Context, groupID *int64, reqModel string, streamStarted bool) {
	...
	logClaudeGPTBridgeRouteDecision(reqLog, decision, reqModel, "selection_race", time.Now())
...
func logClaudeGPTBridgeRouteDecision(reqLog *zap.Logger, decision service.ClaudeGPTBridgeRouteDecision, reqModel, source string, start time.Time) {
	if reqLog == nil {
		return
	}
	fields := []zap.Field{
