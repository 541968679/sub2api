...
func (h *AccountHandler) GetUsage(c *gin.Context) {
	...
	if source == "passive" {
		...
		// active = force upstream balance re-probe when supported (display refresh button).
		usage, err = h.accountUsageService.GetUsage(c.Request.Context(), accountID, true)
	}
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
// MoveToTop pins the account to the top of the admin account list.
// POST /api/v1/admin/accounts/:id/move-to-top
func (h *AccountHandler) MoveToTop(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	account, err := h.adminService.MoveAccountToTop(c.Request.Context(), accountID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// BatchQualityStatsRequest is the body for batch account quality stats.
type BatchQualityStatsRequest struct {
	AccountIDs []int64 `json:"account_ids" binding:"required"`
}

// GetBatchQualityStats returns rolling-window TTFT and success-rate for account list columns.
