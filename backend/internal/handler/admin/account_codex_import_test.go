...
func TestImportCodexSessionHandlerRejectsInvalidConcurrency(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/import/codex-session", bytes.NewBufferString(`{"content":"token","concurrency":-1}`))
	ctx.Request.Header.Set("Content-Type", "application/json")

	(&AccountHandler{}).ImportCodexSession(ctx)
