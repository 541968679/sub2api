...
func (h *GatewayHandler) Messages(c *gin.Context) {
	service.StampGatewayRequestStartAt(c, time.Now())
	...
	for {
		...
		for {
			...
			h.gatewayService.MaybeSetDisplayTokenMultipliers(c.Request.Context(), c, currentAPIKey, reqModel)
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrUserPlatformDailyQuotaExhausted) ||
		...
		return http.StatusTooManyRequests, "rate_limit_exceeded", pkgerrors.Message(err), extractQuotaResetSeconds(err)
	}
	msg := pkgerrors.Message(err)
	if msg == "" {
		logger.L().With(
