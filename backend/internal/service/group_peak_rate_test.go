...
func TestPeakMultiplierAtBoundariesAndInvalidConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if got := g.PeakMultiplierAt(tc.now); got != tc.want {
	...
	for _, invalid := range []*Group{
		...
		if got := invalid.PeakMultiplierAt(peakTestTime(15, 0)); got != 1 {
			t.Fatalf("invalid config got %v", got)
		}
	}
}
...
func TestValidateAndNormalizePeakRateConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if err := ValidatePeakRateConfig(tc.typ, true, tc.start, tc.end, tc.mult); err == nil {
			t.Fatal("expected invalid config")
		}
	}
	enabled, start, end, mult := NormalizePeakRateConfig(SubscriptionTypeStandard, true, "14:00", "18:00", 3)
	if enabled || start != "" || end != "" || mult != 1 {
		t.Fatalf("standard config not cleared: %v %q %q %v", enabled, start, end, mult)
	}
}
...
func TestPeakMultiplierOnlyAffectsTokenBilling(t *testing.T) {
	...
	if math.Abs(text-2.4) > 1e-9 || math.Abs(image-0.8) > 1e-9 {
		t.Fatalf("got text=%v image=%v", text, image)
	}
}

func TestPeakMultiplierDoesNotAffectIndependentVideoRate(t *testing.T) {
	group := newPeakGroup(true, "14:00", "18:00", 3)
	group.VideoRateIndependent = true
	group.VideoRateMultiplier = 0.4
	key := &APIKey{Group: group}
	text, image := computePeakAwareMultipliers(key, 0.8, peakTestTime(15, 0))
	if math.Abs(text-2.4) > 1e-9 || math.Abs(image-0.8) > 1e-9 {
		t.Fatalf("unexpected token/image multipliers: text=%v image=%v", text, image)
	}
	if video := resolveVideoRateMultiplier(key, 0.8); math.Abs(video-0.4) > 1e-9 {
		t.Fatalf("independent video multiplier affected by peak: %v", video)
	}
}
...
func TestPeakMultiplierSnapshotRoundTrip(t *testing.T) {
	...
	if restored.Group == nil || restored.Group.PeakMultiplierAt(peakTestTime(15, 0)) != 3 {
		t.Fatalf("peak snapshot lost: %+v", restored.Group)
	}
}
