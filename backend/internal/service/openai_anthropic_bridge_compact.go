...
// anthropicBridgeCompactResult is the outcome of an optional pre-generation
// compact pass on Anthropic Messages → OpenAI Responses bridge traffic.
// Fail-open: Applied stays false and Body is the original request on any skip/error.
...
// anthropicBridgeActiveSuffixItemCount returns how many Responses input items
// belong to the current turn and must stay outside the compact window.
// Plain user turns keep the last user message; tool-result continuations also
// keep the preceding assistant tool_use message so the tool call pair stays intact.
...
// maybeAutoCompactAnthropicBridge optionally compacts oversized history on the
// Claude→GPT bridge before the generation request is sent.
//
// Fork adaptations vs upstream #4756:
//   - Gate on the mapped upstream model in the Responses body (gpt-5*), not on
//     Claude display names (opus/haiku), so model-mapped bridge traffic is covered.
//   - Skip when the client already initiated a compact request.
//   - Compact model comes from account compact mapping (no global OpenAICompactModel).
//   - Fail-open on any compact error; never blocks generation.
func (s *OpenAIGatewayService) maybeAutoCompactAnthropicBridge(
	...
	// Gate on mapped upstream model after bridge rewrite (body "model"), not the
	// client-facing Claude name. Covers Opus/Haiku → gpt-5.* account mappings.
	upstreamModel := jsonStringValue(requestFields["model"])
	if !isMappedGPT5AnthropicBridgeModel(upstreamModel) || !shouldAutoInjectPromptCacheKeyForCompat(upstreamModel) {
		return result
	}
	...
	if normalizedBody, normalized, normalizeErr := normalizeOpenAICompactReasoningEffortInBody(compactBody); normalizeErr != nil {
...
// isMappedGPT5AnthropicBridgeModel reports whether the mapped upstream model
// is a GPT-5 family model that can use /responses/compact for history shrink.
// Claude display names (opus/haiku/fable) return false; only post-mapping gpt-5* pass.
func isMappedGPT5AnthropicBridgeModel(model string) bool {
	model = strings.ToLower(strings.TrimSpace(lastOpenAIModelSegment(model)))
	if model == "" {
		return false
	}
	// Explicitly reject Claude / fable labels even if they appear after a bad map.
	if strings.HasPrefix(model, "claude-") || strings.Contains(model, "fable") {
		return false
	}
...
// normalizeOpenAICompactReasoningEffortInBody maps compact-unsupported effort
// values (e.g. max → xhigh for non-gpt-5.6) so /responses/compact does not 400.
func normalizeOpenAICompactReasoningEffortInBody(body []byte) ([]byte, bool, error) {
	...
	next, err := sjson.SetBytes(body, "reasoning.effort", normalized)
	if err != nil {
