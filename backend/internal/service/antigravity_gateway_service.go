	...
	if s.settingService != nil && s.settingService.cfg != nil && s.settingService.cfg.Gateway.LogCacheDiagnostics {
		if diag := antigravity.ExtractCacheDiagnostics(geminiBody); diag != nil {
			...
			slog.Info("antigravity_cache_diag",
				...
				"sys_part_hashes", diag.SysPartHashes,
				"sys_part_lens", diag.SysPartLens,
				"sys_part_previews", diag.SysPartPreviews,
				"unstable_part", diag.UnstablePartText,
...
func (s *AntigravityGatewayService) handleUpstreamError(
	...
	if statusCode == 429 {
		if s.rateLimitService != nil && s.rateLimitService.TryHandleOAuthFleetSoft429(ctx, account, statusCode, headers, body) {
			return nil
		}
