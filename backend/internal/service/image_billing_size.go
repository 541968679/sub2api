...
// normalizedImageBillingSizePtr returns a usage-log image size that satisfies
// usage_logs_image_billing_size_check (migration 156): a billing tier whenever
// imageCount > 0, otherwise the trimmed raw value (or nil).
func normalizedImageBillingSizePtr(imageCount int, imageSize string) *string {
	if imageCount > 0 {
		tier := NormalizeImageBillingTierOrDefault(imageSize)
		return &tier
	}
	return optionalTrimmedStringPtr(imageSize)
}
...
// NOTE: upstream additionally ships ApplyOpenAIImageBillingResolution /
// ApplyForwardImageBillingResolution, which depend on ImageInputSize /
// ImageOutputSize(s) / ImageSizeSource / ImageSizeBreakdown forward-result
// fields not yet synced into this fork. Until that sync lands, billing-size
// normalization happens at the usage-log write point in
// openai_gateway_service.go via NormalizeImageBillingTierOrDefault.
