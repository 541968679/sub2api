...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "ZeroCode"),
		...
		ChannelMonitorHideUserRanking:        settings[SettingKeyChannelMonitorHideUserRanking] == "true",
...
func (s *SettingService) GetChannelMonitorRuntime(ctx context.Context) ChannelMonitorRuntime {
	failOpen := ChannelMonitorRuntime{
		Enabled:                true,
...
func (s *SettingService) IsClientDatelineNormalizationEnabled(ctx context.Context) bool {
	return s.getGatewayForwardingSettingsCached(ctx).normalizeDateline
}
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	// Channel monitor feature (default: enabled, 60s, v1)
	...
	result.ChannelMonitorHideUserRanking = settings[SettingKeyChannelMonitorHideUserRanking] == "true"
...
func ParseGatewayNetworkRetryMax(raw string) int {
	raw = strings.TrimSpace(raw)
	if raw == "" {
	...
	for i, rule := range settings.Rules {
		...
		for j, userID := range rule.UserIDs {
			...
			if _, exists := seenUserIDs[userID]; exists {
				return fmt.Errorf("rule[%d]: duplicate user_id %d", i, userID)
			}
