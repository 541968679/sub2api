...
func (s *OpenAIGatewayService) forwardGrokChatCompletionsViaResponses(
	...
	if clientStream {
		result, err = s.handleChatStreamingResponse(resp, c, originalModel, billingModel, upstreamModel, startTime, account)
	} else {
		result, err = s.handleChatBufferedStreamingResponse(resp, c, originalModel, billingModel, upstreamModel, startTime, account)
	}
	if result != nil {
