...
func (a *Account) resolveModelMapping(rawMapping map[string]any) map[string]string {
	if a.Credentials == nil {
		...
		if a.Platform == domain.PlatformAntigravity {
			return domain.ResolveAntigravityDefaultMapping()
		}
...
func (a *Account) IsModelSupported(requestedModel string) bool {
	...
	if a.IsShadow() {
		return mappingSupportsRequestedModel(mapping, requestedModel) || mappingSupportsRequestedModel(mapping, normalizeRequestedModelForLookup(a.Platform, requestedModel))
	}
	if len(mapping) == 0 {
...
func (a *Account) ResolveMappedModelDetailed(requestedModel string) MappedModelResolution {
	...
	if a.Platform == PlatformGrok && !a.hasExplicitModelMapping() {
		if mappedModel, mappingKey, matched := resolveRequestedModelInMappingWithKey(xai.DefaultModelMapping(), requestedModel); matched {
			result.MappedModel = mappedModel
			result.Matched = true
			result.Source = ModelMappingSourcePlatformDefault
			result.MappingKey = mappingKey
...
func (a *Account) openAIEndpointCapabilitySet() (map[string]bool, bool) {
	...
	// 空容器（{} / []）与未配置一致：不限制任何能力。
	// 避免 OAuth 账号因 API 直写/导入/历史数据遗留的空对象而被调度器静默排除。
...
func (a *Account) SupportsOpenAIImageCapability(capability OpenAIImagesCapability) bool {
	...
	case OpenAIImagesCapabilityBasic, OpenAIImagesCapabilityNative:
		if !a.OpenAIImagesEndpointEnabled() {
			return false
		}
