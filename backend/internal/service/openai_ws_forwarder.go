...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	// Grok has no upstream Responses WebSocket; always bridge client WS → HTTP/SSE.
	...
	dedicatedMode := modeRouterV2Enabled && !forceHTTPBridge && ingressMode == OpenAIWSIngressModeDedicated
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		//
		// Model fallback: first turn still requires model; follow-up
		// response.create frames may omit it and then reuse
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		logOpenAIWSModeInfo(
			"ingress_ws_http_bridge_start account_id=%d account_type=%s platform=%s payload_bytes=%d threshold_bytes=%d has_session_hash=%v store_disabled=%v force_http_bridge=%v",
			account.ID,
			account.Type,
			account.Platform,
		...
		for turn := 1; ; turn++ {
			...
			if failoverInputErr != nil {
				return fmt.Errorf("build websocket current-turn failover input: %w", failoverInputErr)
			}
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string) (*OpenAIForwardResult, error) {
		...
		turnHasFunctionCallOutput := ValidateFunctionCallOutputContextBytes(payload).HasFunctionCallOutput
		eventCount := 0
		tokenEventCount := 0
		terminalEventCount := 0
		...
		for {
			...
			if !clientDisconnected {
				if openAIWSEventMayContainModel(eventType) {
					observer := upstreamResponseModelObserverFromContext(c)
					if observer != nil {
						observer.ObserveOpenAI(upstreamMessage, eventType)
					}
				}
	...
	for {
		...
		toolSignals := ToolContinuationSignals{
			HasFunctionCallOutput: ValidateFunctionCallOutputContextBytes(currentPayload).HasFunctionCallOutput,
		}
...
func openAIWSAPIKeyFromContext(c *gin.Context) *APIKey {
	if c == nil {
		return nil
	}
	value, exists := c.Get("api_key")
...
func openAIWSGroupAllowsImageGeneration(apiKey *APIKey) bool {
	if apiKey == nil || apiKey.Group == nil {
		return true
	}
	return apiKey.Group.AllowImageGeneration
}
...
func (s *OpenAIGatewayService) SelectAccountByPreviousResponseID(
	...
	if shouldClearStickySession(account, requestedModel) || !account.IsOpenAI() || !account.IsSchedulable() || !s.admitsScheduleUser(ctx, account) {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return nil, nil
	}
	...
	result, pairFull, acquireErr := s.tryAcquireAccountAndPairSlot(ctx, account)
	if pairFull {
		return nil, nil
	}
