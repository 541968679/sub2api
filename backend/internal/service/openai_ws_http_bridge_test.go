...
type openAIWSHTTPBridgeTestUpstream struct {
	resp      *http.Response
...
func (u *openAIWSHTTPBridgeTestUpstream) Do(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	u.lastReq = req
	...
	return &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`data: {"type":"response.completed","response":{"id":"resp_default"}}` + "\n\n")),
	}, nil
}

func (u *openAIWSHTTPBridgeTestUpstream) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestOpenAIWSHTTPBridgeRelaysSSEFramesAsWebSocketMessages(t *testing.T) {
	...
	upstream := &openAIWSHTTPBridgeTestUpstream{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
...
func TestOpenAIWSHTTPBridgeKeepsContinuationFramesOnHTTPWithoutPreviousResponseID(t *testing.T) {
	...
	upstream := &openAIWSHTTPBridgeTestUpstream{responses: []*http.Response{
		{
			StatusCode: http.StatusOK,
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"},
