...
func TestStartSlotCleanupWorker_UsesExistingRedisKeysWithoutAccountRepo(t *testing.T) {
	...
	for {
		...
		select {
		case <-deadline:
			t.Fatal("cleanup worker did not scan existing Redis account slot keys")
		case <-ticker.C:
		}
	}
}
