...
func TestGroupModelAllowlistEnforceDefaultsOff(t *testing.T) {
	var nilSvc *SettingService
	if nilSvc.IsGroupModelAllowlistEnforceEnabled(context.Background()) {
		t.Fatal("missing setting service must not enforce gateway allowlists")
	}
	if (&SettingService{}).IsGroupModelAllowlistEnforceEnabled(context.Background()) {
		t.Fatal("empty setting service must not enforce gateway allowlists")
	}
}
