...
type SystemSettings struct {
	...
	OpenAINewAPISlimCompleted           bool    // 是否精简 Responses completed（默认 false；兼容 NewAPI 空 output）
	OpenAINewAPISlimCompletedUserIDs    []int64 // 全站关闭时仍精简 completed 的用户 ID 白名单
	...
	// DisplayContextTokenMax / DisplayOutputTokenMax: 0 = off (code default).
	DisplayContextTokenMax int64
	DisplayOutputTokenMax  int64
