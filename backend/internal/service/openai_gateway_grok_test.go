...
func grokSSECompletedResponse(responseID, model string) *http.Response {
	...
	return &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_grok"}},
...
func TestForwardAsAnthropicForGrokUsesXAIResponses(t *testing.T) {
	...
	c.Request.Header.Set("OpenAI-Beta", "grok-beta")
	...
	upstream := &httpUpstreamRecorder{resp: grokSSECompletedResponse("resp_grok_messages", "grok-4.3")}
	svc := &OpenAIGatewayService{
		httpUpstream:      upstream,
		grokTokenProvider: NewGrokTokenProvider(repo, nil),
		accountRepo:       repo,
	...
	require.Equal(t, "Bearer access-token", upstream.lastReq.Header.Get("Authorization"))
	require.Equal(t, "sub2api-grok/1.0", upstream.lastReq.Header.Get("User-Agent"))
	require.Equal(t, "grok-beta", upstream.lastReq.Header.Get("OpenAI-Beta"))
...
func TestHandleGrokAccountUpstreamError429DoesNotShortenExistingPause(t *testing.T) {
	...
	require.WithinDuration(t, time.Now().Add(45*time.Second), repo.rateLimitCalls[0], time.Second)
...
func TestUpdateGrokUsageSnapshotPersistsSuccessfulQuotaExhaustion(t *testing.T) {
	...
	require.Len(t, repo.rateLimitCalls, 1)
	require.WithinDuration(t, resetAt, repo.rateLimitCalls[0], time.Second)
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
