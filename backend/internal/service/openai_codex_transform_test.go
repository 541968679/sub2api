...
func TestEnsureOpenAIResponsesImageGenerationTool_SkipsClientImageGenNamespace(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.4",
		"tools": []any{
			map[string]any{
...
func TestStripOpenAIImageGenerationTools_StripsNamespaceShapesOnly(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "custom", customTool["type"])
	codeTool, ok := tools[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "code_tools", codeTool["name"])

	input, ok := reqBody["input"].([]any)
	require.True(t, ok)
	require.Len(t, input, 2)
	additionalItem, ok := input[1].(map[string]any)
	require.True(t, ok)
	additional, ok := additionalItem["tools"].([]any)
	require.True(t, ok)
	require.Len(t, additional, 1)
	browserTool, ok := additional[0].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "browser_tools", browserTool["name"])
...
func TestApplyCodexOAuthTransform_PreservesClientImageGenNamespace(t *testing.T) {
	imageNamespace := map[string]any{
		"type":        "namespace",
		"name":        "image_gen",
		"description": "Tools in the image_gen namespace.",
		"tools": []any{
	...
	require.Equal(t, "image_gen", choice["name"])
	require.True(t, hasOpenAIImageGenerationTool(reqBody))
	input, ok := reqBody["input"].([]any)
	require.True(t, ok)
	require.Len(t, input, 2)
