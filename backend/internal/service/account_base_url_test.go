...
func TestGetGrokBaseURLUsesSubscriptionProxyForOAuth(t *testing.T) {
	tests := []struct {
		...
		{
			name: "oauth legacy API default is migrated at runtime",
			account: Account{
				Type:     AccountTypeOAuth,
				Platform: PlatformGrok,
				Credentials: map[string]any{
		...
		{
			name: "oauth canonical API URL variants are migrated at runtime",
			account: Account{
				Type:     AccountTypeOAuth,
				Platform: PlatformGrok,
				Credentials: map[string]any{
					"base_url": "HTTPS://API.X.AI:443/v1/",
				},
			},
			expected: xai.DefaultCLIBaseURL,
		},
		{
			name: "oauth custom base_url remains supported",
			account: Account{
				Type:     AccountTypeOAuth,
				Platform: PlatformGrok,
				Credentials: map[string]any{
		...
		{
			name: "oauth suspicious official-looking URL remains custom",
			account: Account{
				Type:     AccountTypeOAuth,
				Platform: PlatformGrok,
				Credentials: map[string]any{
					"base_url": "https://api.x.ai@proxy.example/v1",
				},
			},
			expected: "https://api.x.ai@proxy.example/v1",
		},
		{
		...
		{
			name: "API key official base_url does not switch to subscription proxy",
			account: Account{
				Type:     AccountTypeAPIKey,
				Platform: PlatformGrok,
				Credentials: map[string]any{
