...
func TestUpstreamResponseModelObservationAttemptReset(t *testing.T) {
	...
	require.False(t, second.Conflict())
}
...
func TestUpstreamModelMismatchDoesNotCollapseDifferentModels(t *testing.T) {
	tests := []struct {
		...
		{name: "gpt-6-astra vs gpt-5.6-luna", sentModel: "gpt-6-astra", responseModel: "gpt-5.6-luna"},
	}
...
func TestObserveAntigravityGeminiSSELineReadsWrapperModelWithoutUnwrap(t *testing.T) {
	tests := []struct {
		...
		{name: "outer declaration takes precedence", payload: `{"modelVersion":"gemini-outer","response":{"modelVersion":"gemini-inner","candidates":[]}}`, want: "gemini-outer"},
	}
...
func TestAttachUpstreamResponseModelAudit(t *testing.T) {
	log := &UsageLog{}
	attachUpstreamResponseModelAudit(log, "gpt-6-astra", "gpt-5.6-luna")
	require.NotNil(t, log.UpstreamResponseModel)
	require.Equal(t, "gpt-5.6-luna", *log.UpstreamResponseModel)
	require.NotNil(t, log.UpstreamModelMismatch)
	require.True(t, *log.UpstreamModelMismatch)

	matched := &UsageLog{}
	attachUpstreamResponseModelAudit(matched, "claude-sonnet-4-20250514", "claude-sonnet-4-20250514")
	require.NotNil(t, matched.UpstreamModelMismatch)
	require.False(t, *matched.UpstreamModelMismatch)

	empty := &UsageLog{}
	attachUpstreamResponseModelAudit(empty, "gpt-6-astra", "")
	require.Nil(t, empty.UpstreamResponseModel)
	require.Nil(t, empty.UpstreamModelMismatch)
}
