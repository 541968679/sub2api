...
func (s *adminServiceImpl) deleteUserWithAPIKeys(ctx context.Context, userID int64, apiKeys []APIKey) error {
	if s.apiKeyRepo != nil {
		auditDeleter, hasAuditDelete := s.apiKeyRepo.(interface {
		...
		for _, key := range apiKeys {
			...
			var err error
			if hasAuditDelete {
				err = auditDeleter.DeleteWithAudit(ctx, key.ID)
			} else {
				err = s.apiKeyRepo.Delete(ctx, key.ID)
			}
			if err != nil {
...
func (s *adminServiceImpl) BatchSetGroupRateMultipliers(ctx context.Context, groupID int64, entries []GroupRateMultiplierInput) error {
	...
	for _, e := range entries {
		...
		if e.DisplayRateMultiplier != nil && *e.DisplayRateMultiplier <= 0 {
			return fmt.Errorf("display_rate_multiplier must be > 0 (user_id=%d)", e.UserID)
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if account.IsShadow() && input.Credentials != nil {
		...
		account.Credentials = preserveOAuthSecretCredentials(account.Credentials, input.Credentials)
		// 校验并规范化请求头覆写配置（header 名小写化、格式检查）
