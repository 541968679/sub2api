...
func (BatchImageJob) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("parent_batch_id").Optional().Nillable().MaxLen(64),
		...
		field.Float("group_rate_multiplier").SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).Default(1),
		field.Float("account_rate_multiplier").SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).Default(1),
