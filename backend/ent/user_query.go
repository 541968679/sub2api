...
// QueryScheduledAccounts chains the current query on the "scheduled_accounts" edge.
func (_q *UserQuery) QueryScheduledAccounts() *AccountQuery {
...
func (_q *UserQuery) QueryAccountScheduleUsers() *AccountScheduleUserQuery {
...
// WithScheduledAccounts tells the query-builder to eager-load the nodes that are connected to
// the "scheduled_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithScheduledAccounts(opts ...func(*AccountQuery)) *UserQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withScheduledAccounts = query
	return _q
}
...
func (_q *UserQuery) WithAccountScheduleUsers(opts ...func(*AccountScheduleUserQuery)) *UserQuery {
	query := (&AccountScheduleUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withScheduledAccounts; query != nil {
		if err := _q.loadScheduledAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.ScheduledAccounts = []*Account{} },
			func(n *User, e *Account) { n.Edges.ScheduledAccounts = append(n.Edges.ScheduledAccounts, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadScheduledAccounts(ctx context.Context, query *AccountQuery, nodes []*User, init func(*User), assign func(*User, *Account)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*User)
	nids := make(map[int64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ScheduledAccountsTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(user.ScheduledAccountsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.ScheduledAccountsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ScheduledAccountsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "scheduled_accounts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadAccountScheduleUsers(ctx context.Context, query *AccountScheduleUserQuery, nodes []*User, init func(*User), assign func(*User, *AccountScheduleUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountscheduleuser.FieldUserID)
	}
