...
type UserEdges struct {
	...
	// ScheduledAccounts holds the value of the scheduled_accounts edge.
	ScheduledAccounts []*Account `json:"scheduled_accounts,omitempty"`
...
// ScheduledAccountsOrErr returns the ScheduledAccounts value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) ScheduledAccountsOrErr() ([]*Account, error) {
	if e.loadedTypes[6] {
		return e.ScheduledAccounts, nil
	}
	return nil, &NotLoadedError{edge: "scheduled_accounts"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldEmail, user.FieldPasswordHash, user.FieldRole, user.FieldStatus, user.FieldUsername, user.FieldNotes, user.FieldTotpSecretEncrypted, user.FieldSignupSource, user.FieldBalanceNotifyThresholdType, user.FieldBalanceNotifyExtraEmails, user.FieldDownstreamUsageTokenMode:
			values[i] = new(sql.NullString)
		case user.FieldCreatedAt, user.FieldUpdatedAt, user.FieldDeletedAt, user.FieldTotpEnabledAt, user.FieldLastLoginAt, user.FieldLastActiveAt, user.FieldPinnedAt:
...
// QueryScheduledAccounts queries the "scheduled_accounts" edge of the User entity.
func (_m *User) QueryScheduledAccounts() *AccountQuery {
	return NewUserClient(_m.config).QueryScheduledAccounts(_m)
}
