...
func (_c *UserCreate) AddScheduledAccountIDs(ids ...int64) *UserCreate {
	_c.mutation.AddScheduledAccountIDs(ids...)
	return _c
}

// AddScheduledAccounts adds the "scheduled_accounts" edges to the Account entity.
func (_c *UserCreate) AddScheduledAccounts(v ...*Account) *UserCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddScheduledAccountIDs(ids...)
}
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.ScheduledAccountsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
