...
func (tx *Tx) init() {
	...
	tx.AccountQualitySnapshot = NewAccountQualitySnapshotClient(tx.config)
	tx.AccountScheduleUser = NewAccountScheduleUserClient(tx.config)
	...
	tx.ImageChannelMonitor = NewImageChannelMonitorClient(tx.config)
	tx.ImageChannelMonitorHistory = NewImageChannelMonitorHistoryClient(tx.config)
