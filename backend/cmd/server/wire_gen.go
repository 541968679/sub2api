...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userSmartScheduleRepository := repository.NewUserSmartScheduleRepository(client)
	userSmartScheduleCache := repository.NewUserSmartScheduleCache(redisClient, userSmartScheduleRepository)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache, userSmartScheduleCache)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService, accountRepository)
	apiKeyAuthCacheInvalidator := service.ProvideAPIKeyAuthCacheInvalidator(apiKeyService)
	promoService := service.NewPromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator)
	usageLogRepository := repository.NewUsageLogRepository(client, db)
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, userGroupRateRepository, usageLogRepository)
	...
	authService := service.NewAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, userPlatformQuotaServiceAdapter)
	...
	userHandler := handler.NewUserHandler(userService, authService, emailService, emailCache, affiliateService, userPlatformQuotaServiceAdapter)
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyService)
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator)
	globalModelPricingRepository := repository.NewGlobalModelPricingRepository(db)
	globalPricingCache := service.NewGlobalPricingCache(globalModelPricingRepository)
	...
	userModelPricingRepository := repository.NewUserModelPricingRepository(db)
	globalModelPricingService := service.NewGlobalModelPricingService(globalModelPricingRepository, globalPricingCache, pricingService, channelService, groupRepository, userModelPricingRepository)
	modelPlazaService := service.NewModelPlazaService(groupRepository, globalModelPricingService, pricingService)
	userModelPricingService := service.NewUserModelPricingService(userModelPricingRepository)
	...
	_ = service.ProvideChannelMonitorV2Aggregator(channelMonitorV2Repository, db, settingService)
	...
	accountQualityLiveCache := repository.NewAccountQualityLiveCache(redisClient)
	publicScheduleQualityCache := repository.NewPublicScheduleQualityCache(redisClient)
	publicScheduleQualityService := service.ProvidePublicScheduleQualityService(publicScheduleQualityCache, settingService, accountRepository)
	userSmartScheduleService := service.NewUserSmartScheduleService(userSmartScheduleRepository, userSmartScheduleCache, accountRepository, accountQualityLiveCache, concurrencyService)
	adminService := service.ProvideAdminService(userRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userModelPricingRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, accountQualityLiveCache)
	sessionLimitCache := repository.ProvideSessionLimitCache(redisClient, configConfig)
	rpmCache := repository.NewRPMCache(redisClient)
	groupCapacityAPIKeyIDLister := repository.NewGroupCapacityAPIKeyLister(db)
	groupCapacityService := service.NewGroupCapacityService(accountRepository, groupRepository, concurrencyService, sessionLimitCache, rpmCache, groupCapacityAPIKeyIDLister)
	groupHandler := admin.NewGroupHandler(adminService, dashboardService, groupCapacityService)
	claudeOAuthClient := repository.NewClaudeOAuthClient()
	oAuthService := service.NewOAuthService(proxyRepository, claudeOAuthClient)
	openAIOAuthClient := repository.NewOpenAIOAuthClient()
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, provideEphemeralPrivacyClientFactory())
	...
	oauthFleetSoft429Cache := repository.NewOAuthFleetSoft429Cache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator, oauthFleetSoft429Cache)
	...
	antigravityQuotaFetcher := service.ProvideAntigravityQuotaFetcher(proxyRepository, antigravityTokenProvider)
	antigravityCreditSampleRepository := repository.NewAntigravityCreditSampleRepository(client, db)
	antigravityCreditSampler := service.NewAntigravityCreditSampler(antigravityCreditSampleRepository, antigravityQuotaFetcher)
	...
	schedulePnlService := service.NewSchedulePnlService(usageLogRepository, userSmartScheduleRepository)
	adminUserHandler := admin.NewUserHandler(adminService, concurrencyService, userPlatformQuotaServiceAdapter, billingCache, accountUsageService, userSmartScheduleService, schedulePnlService)
	...
	antigravityGatewayService := service.NewAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, settingService, internal500CounterCache, antigravityCreditSampler)
	...
	apiKeyService.SetUpstreamModelsFetcher(accountTestService)
	...
	accountQualityMaintenanceService := service.ProvideAccountQualityMaintenanceService(accountQualitySnapshotRepository, userQualitySnapshotRepository, usageLogRepository, timingWheelService, leaderLockCache, db, accountRepository, settingService, accountQualityLiveCache, publicScheduleQualityService)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, gatewayCache, rpmCache, compositeTokenCacheInvalidator, settingService, accountQualityMaintenanceService)
	...
	batchImageWorkerRuntime := service.ProvideBatchImageWorkerRuntime(batchImageRepository, accountRepository, batchImageQueue, usageBillingRepository, usageLogRepository, batchImageModelPricingResolver, apiKeyAuthCacheInvalidator, configConfig, modelPricingResolver)
	balanceNotifyService := service.ProvideBalanceNotifyService(emailService, settingRepository, accountRepository)
	gatewayService := service.ProvideGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, balanceNotifyService, antigravityCreditSampler, accountQualityLiveCache, userSmartScheduleService, accountQualityMaintenanceService, publicScheduleQualityService)
	openAITokenProvider := service.ProvideOpenAITokenProvider(accountRepository, geminiTokenCache, openAIOAuthService, oAuthRefreshAPI)
	accountTestService.SetOpenAITokenProvider(openAITokenProvider)
	...
	accountUsageService.SetOpenAIQuotaService(openAIQuotaService)
	...
	rateLimitService.SetOpenAI7dLiteLLMCycles(openAI7dLiteLLMCycleService)
	openAIGatewayService.SetOpenAI7dLiteLLMCycles(openAI7dLiteLLMCycleService)
	geminiMessagesCompatService := service.ProvideGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig, accountQualityLiveCache, userSmartScheduleService, publicScheduleQualityService)
	opsSystemLogSink := service.ProvideOpsSystemLogSink(opsRepository)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, userSmartScheduleService, accountQualityMaintenanceService)
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService, globalModelPricingService, userModelPricingService, modelPricingResolver, opsService, settingService)
	...
	modelPricingHandler := handler.ProvideModelPricingHandler(globalModelPricingService, settingService, accountRepository)
	...
	tutorialPageHandler := admin.NewTutorialPageHandler(settingRepository)
	purchasePageAdminHandler := admin.NewPurchasePageAdminHandler(settingRepository)
	redeemPageAdminHandler := admin.NewRedeemPageAdminHandler(settingRepository)
	...
	imageChannelMonitorService := service.ProvideImageChannelMonitorService(imageChannelMonitorRepository, accountRepository, proxyRepository, secretEncryptor, httpUpstream, tlsFingerprintProfileService, apiKeyService, groupRepository, configConfig)
	imageChannelMonitorHandler := admin.NewImageChannelMonitorHandler(imageChannelMonitorService)
	channelLoadtestService := service.NewChannelLoadtestService(accountRepository, proxyRepository)
	channelLoadtestHandler := admin.NewChannelLoadtestHandler(channelLoadtestService)
	imageChannelMonitorUserHandler := handler.NewImageChannelMonitorUserHandler(imageChannelMonitorService, settingService)
	...
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, globalModelPricingService, userModelPricingService, modelPricingResolver, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService)
	...
	handlerPricingPageHandler := handler.NewPricingPageHandler(globalModelPricingService, userModelPricingService, settingRepository)
	handlerTutorialPageHandler := handler.NewTutorialPageHandler(settingRepository)
	handlerPurchasePageHandler := handler.NewPurchasePageHandler(settingRepository)
	handlerRedeemPageHandler := handler.NewRedeemPageHandler(settingRepository)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	modelPlazaHandler := handler.NewModelPlazaHandler(modelPlazaService, userService, settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, distributionHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, imageChannelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, handlerPricingPageHandler, handlerTutorialPageHandler, handlerPurchasePageHandler, handlerRedeemPageHandler, availableChannelHandler, batchImageHandler, modelPlazaHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	opsCleanupService := service.ProvideOpsCleanupService(opsRepository, db, redisClient, configConfig, channelMonitorService, imageChannelMonitorService)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, leaderLockCache, db)
...
func provideCleanup(
	...
	return func() {
		...
		parallelSteps := []cleanupStep{
			...
			{"ImageChannelMonitorRunner", func() error {
				if imageChannelMonitorRunner != nil {
					imageChannelMonitorRunner.Stop()
				}
				return nil
			}},
