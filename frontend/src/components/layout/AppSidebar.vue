...
interface NavItem {
  ...
  expandOnly?: boolean
  /**
   * Optional feature-flag getter. Items are hidden only when it returns false;
   * undefined keeps the item visible while public settings are still loading.
   */
...
const BatchImageIcon = {
  render: () =>
    h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M3.75 6.75A2.25 2.25 0 016 4.5h12A2.25 2.25 0 0120.25 6.75v10.5A2.25 2.25 0 0118 19.5H6a2.25 2.25 0 01-2.25-2.25V6.75zM3.75 15l3.879-3.879a2.25 2.25 0 013.182 0L13.5 13.81l1.629-1.629a2.25 2.25 0 013.182 0l1.939 1.939M15.75 8.25h.008v.008h-.008V8.25z' })
    ])
}
...
// which handles feature flag fallback while settings are loading.
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/model-plaza', label: t('nav.modelPlaza'), icon: PriceTagIcon, featureFlag: flagModelPlaza },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/plugins', label: t('nav.plugins'), icon: ServerIcon, featureFlag: flagPluginManagement },
    {
      path: '/admin/channels',
    ...
    { path: '/admin/distribution', label: t('nav.distributionManagement'), icon: CreditCardIcon, hideInSimpleMode: true },
    {
  ...
  // In simple mode, keep API keys available before system settings.
