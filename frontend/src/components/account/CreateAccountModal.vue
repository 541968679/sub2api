  ...
  >
    ...
    >
      ...
      <div v-if="form.platform === 'antigravity'" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div>
          ...
          <div v-if="antigravityAccountType === 'upstream'" class="mb-3 flex flex-wrap gap-2">
            <button
              type="button"
              @click="syncAntigravityPreviewModels"
              :disabled="isSyncingAntigravityPreview"
              class="rounded-lg border border-emerald-200 px-3 py-1.5 text-sm text-emerald-600 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-800 dark:text-emerald-400 dark:hover:bg-emerald-900/30"
            >
              {{ isSyncingAntigravityPreview ? t('admin.accounts.syncUpstreamModelsLoading') : t('admin.accounts.syncUpstreamModels') }}
            </button>
          </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <button
            type="button"
            data-testid="create-openai-claude-gpt-bridge-toggle"
        ...
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                type="button"
                data-testid="edit-openai-claude-gpt-bridge-template"
          ...
          <div v-if="showOpenAIClaudeGPTBridgeTemplateEditor" class="mt-3 space-y-2">
            ...
            >
              <input
                ...
                :data-testid="`openai-claude-gpt-bridge-template-from-${index}`"
                :placeholder="t('admin.accounts.requestModel')"
              />
              <input
                v-model="mapping.to"
                type="text"
                class="input flex-1 text-xs"
                :data-testid="`openai-claude-gpt-bridge-template-to-${index}`"
                :placeholder="t('admin.accounts.actualModel')"
              />
              <button
                type="button"
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                data-testid="save-openai-claude-gpt-bridge-template"
        ...
        <div class="flex items-center justify-between gap-4">
          ...
          <div class="w-52 shrink-0">
            <Select
              ...
              data-testid="create-openai-responses-mode"
    ...
    <div v-else class="space-y-5">
      <OAuthAuthorizationFlow
        ...
        :show-refresh-token-option="form.platform === 'openai' || form.platform === 'antigravity' || form.platform === 'grok' || (form.platform === 'gemini' && geminiOAuthType === 'google_one')"
...
const openAIClaudeGPTBridgeTemplateDraft = ref<ModelMapping[]>([])
...
const isSyncingAntigravityPreview = ref(false)
...
watch(
  ...
  (newPlatform) => {
    ...
    if (newPlatform === 'grok' && accountCategory.value !== 'apikey') {
      accountCategory.value = 'oauth-based'
...
const syncAntigravityPreviewModels = async () => {
  if (isSyncingAntigravityPreview.value) return
  ...
  isSyncingAntigravityPreview.value = true
  try {
    ...
    for (const model of upstreamModels) {
      if (!hasAntigravityMappingFor(model)) {
    ...
    isSyncingAntigravityPreview.value = false
  }
}
...
const handleSubmit = async () => {
  ...
  if (isOAuthFlow.value) {
    if (!useEmailAsName.value && !form.name.trim()) {
...
// OpenAI OAuth 授权码兑换
const buildGrokCredentials = (tokenInfo: Parameters<typeof grokOAuth.buildCredentials>[0]) => {
...
const handleGrokValidateRT = async (refreshTokenInput: string) => {
  ...
  for (const [index, refreshToken] of refreshTokens.entries()) {
    const tokenInfo = await grokOAuth.validateRefreshToken(refreshToken, form.proxy_id)
    if (!tokenInfo) {
      failures.push(`#${index + 1}: ${grokOAuth.error.value}`)
      continue
    }
    try {
      await adminAPI.accounts.create({
        name: refreshTokens.length > 1
          ? `${form.name || tokenInfo.email || 'Grok OAuth'} #${index + 1}`
          : form.name || tokenInfo.email || 'Grok OAuth',
        notes: form.notes,
        platform: 'grok',
        type: 'oauth',
      ...
      failures.push(`#${index + 1}: ${error?.message || t('admin.accounts.failedToCreate')}`)
    }
  }
  ...
  if (failures.length > 0) grokOAuth.error.value = failures.join('\n')
  if (successCount > 0 && failures.length === 0) handleClose()
}

const handleGrokExchange = async (authCode: string) => {
  const state = (oauthFlowRef.value?.oauthState || grokOAuth.state.value || '').trim()
  if (!authCode.trim() || !grokOAuth.sessionId.value || !state) return
