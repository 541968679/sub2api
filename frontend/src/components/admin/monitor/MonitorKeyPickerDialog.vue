            ...
            >
              ...
              <td class="px-3 py-2">
                <GroupBadge
                  ...
                  :user-rate-multiplier="userRateMultiplier(k.group.id)"
                />
...
import type { ApiKey, UserGroupRateData } from '@/types'
...
const props = withDefaults(defineProps<{
  ...
  keys: ApiKey[]
  provider: Provider
  userGroupRates?: Record<number, UserGroupRateData>
}>(), {
  userGroupRates: () => ({}),
})
...
function userRateMultiplier(groupId: number): number | null {
  const data = props.userGroupRates[groupId]
  if (!data) return null
