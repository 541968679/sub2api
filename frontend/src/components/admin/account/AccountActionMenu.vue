        ...
        <div class="py-1">
          <template v-if="account">
            ...
            <template v-if="!isShadow && (account.type === 'oauth' || account.type === 'setup-token')">
              ...
              <button v-if="account.type === 'oauth'" @click="$emit('update-refresh-token', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
                <Icon name="key" size="sm" />
                {{ t('admin.accounts.updateRefreshToken') }}
              </button>
            </template>
            ...
            <button v-if="canCreateSparkShadow" @click="$emit('create-spark-shadow', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-amber-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="link" size="sm" />
...
const emit = defineEmits(['close', 'test', 'move-to-top', 'stats', 'schedule', 'reauth', 'refresh-token', 'update-refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'export-codex', 'create-spark-shadow', 'clear-concurrency'])
const { t } = useI18n()
...
const canCreateSparkShadow = computed(() => isOpenAIOAuth.value && !isShadow.value)
const isOpenAICodexExportCandidate = computed(() => {
  if (isShadow.value) return false
