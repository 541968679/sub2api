  ...
  admin: {
    ...
    users: {
      ...
      smartSchedule: {
        ...
        pairCap: '配对并发',
        pairCapHint: '该用户在此账号上的并发上限。空表示不额外限制，也不会占用配对 Redis 槽；有上限时徽章是当前占用 / 配对上限。改上限后需点保存。',
        uncappedPair: '不额外限制',
        ...
        autoSort: '自动排序',
        autoSortHint:
          '按入池状态 → 配对剩余 → 账号并发 → 原优先级 → LRU 重排当前平台池成员，并立即写入账号优先级（数值越小越优先，与 Claude / 同倍率 OpenAI 一致）。不按上游倍率排序；热路径已有倍率叠加。不删成员。',
        autoSortProgress: '正在写入优先级 {done}/{total}…',
        autoSortSuccess: '已按自动排序写入 {count} 个账号的优先级',
        autoSortUnchanged: '当前优先级已符合自动排序，无需写入',
        autoSortPartial: '已写入 {written} 个账号的优先级，{failed} 个失败',
        autoSortFailed: '自动排序写入优先级失败',
        refresh: '刷新',
        userCore: '用户核心参数',
