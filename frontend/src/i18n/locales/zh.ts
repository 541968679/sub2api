  ...
  admin: {
    ...
    accounts: {
      ...
      userSchedule: {
        ...
        modeAllow: '允许名单',
        modeDeny: '拒绝名单',
        hint: '允许名单、拒绝名单和对级并发是三套独立配置，可重叠。拒绝优先于允许；未设上限则只受账号并发和用户全局并发限制。',
        allowHint: '非空时只有名单内用户可调度到该账号。',
        denyHint: '命中即不可调度，忽略对级上限。',
        concurrencyHint: '该用户在此账号上的最大并发。空或 0 表示不对这一对额外限流。最小显式值为 1。',
        capOnly: '仅设对级上限',
        capOnlyHint: '用户不必出现在允许/拒绝名单里，也可以单独设置对级上限。',
        ...
        overwriteHint: '勾选后覆盖所选账号对应名单（不是合并）。批量不会改对级并发。',
        deleted: '已删除',
