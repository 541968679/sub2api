  ...
  admin: {
    ...
    users: {
      ...
      smartSchedule: {
        ...
        schedWindowN: '窗口条数',
        schedWindowNHint: '调度期（可调度）首字/耗时评定窗口长度。满窗后才评超标条数与中位数。留空且已配首字中位数门槛时默认 20。',
        schedMaxSlowInWindow: '窗口内超标条数',
        schedMaxSlowInWindowHint: '调度期：满窗口后超标条数 ≥ 此值即冷却（原 K）。留空且已配首字中位数门槛时默认 6。',
        schedMaxConsecutiveSlow: '连续超标条数',
        schedMaxConsecutiveSlowHint: '调度期：满窗口后末尾连续此条数均超标才冷却，禁止未满窗评连续（原 C）。留空且已配首字中位数门槛时默认 3。',
