  ...
  admin: {
    ...
    accounts: {
      ...
      userSchedule: {
        ...
        modeAllow: '允许名单',
        modeDeny: '拒绝名单',
        hint: '允许名单、拒绝名单和用户并发上限是三套独立配置，可重叠。拒绝优先于允许；未设上限则只受账号并发和用户全局并发限制。',
        ...
        restoreDefault: '恢复默认',
        restoreDefaultHint: '清空允许名单、拒绝名单和对级并发',
        enableWrite: '写入用户调度',
        enableAllowWrite: '覆盖允许名单',
        enableDenyWrite: '覆盖拒绝名单',
        overwriteHint: '勾选后覆盖所选账号对应名单（不是合并）。批量不会改用户并发上限。',
        deleted: '已删除',
    ...
    settings: {
      ...
      qualityHardClose: {
        title: '质量硬关闭',
        description: '系统总开关与默认阈值模板。保存不会自动打开任何账号，账号需在稳定性窗口中单独开启。',
        enabled: '启用质量硬关闭总开关',
        enabledHint: '关闭时即使账号已开启也不会评估。打开后只评估同时开启的账号。',
        templateHint: '以下为账号选择“使用系统默认模板”时继承的阈值。保存此卡片不会启用任何账号。',
        maxP50: '最高 p50 首字（毫秒）',
        minSuccessRate: '最低成功率（%）',
