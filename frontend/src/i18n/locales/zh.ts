  ...
  admin: {
    ...
    users: {
      ...
      smartSchedule: {
        ...
        pairQuality: '配对质量',
        pairQualityHint: '仅本用户 × 本账号的最近 N 条窗口：首字 p50、成功率、两窗条数/N。点击打开配对趋势与冷却/豁免期/可调度事件。不是账号 15 分钟质量。',
        ...
        pairTrendEmptyHint: '配对质量每完成一条入窗后重算。这里不是账号 5 分钟质量历史。',
        pairEventsTitle: '冷却 / 豁免期 / 可调度',
        ...
        resumeFailed: '恢复配对失败',
        switchState: '切换状态',
        switchStateHint: '手动切换该配对入池状态：暂停、配对冷却、豁免期或可调度',
        ...
        admission: '入池状态',
        admissionHint: '这是该用户在本池里能不能被选中，不是账号 status。暂停和配对冷却是真锁；已保存的配对质量门槛未达标但尚未写入冷却时显示「下次请求将冷却」；未保存或未启用的门槛只是预览。',
        ...
        admissionResumedHint: '豁免期内完全不判断、不写冷却。期满后再按配对窗口判断。',
        filterSearch: '搜索池内账号',
    ...
    accounts: {
      ...
      userSchedule: {
        ...
        qualityGate: '质量门槛',
        qualityGateHint: '账号当前 15 分钟首字/成功率不满足时，只停止把这个账号调度给该用户。未填的指标不判；样本不够不判。',
        qualityTemplateHint: '模板全站只有一份，不绑定某个用户。保存模板会覆盖旧模板；应用模板只填入当前用户的门槛表单，需再保存账号才会生效。',
        qualityNoPauseHint: '用户门槛没有暂停时间：窗口不达标就停调给该用户。点「立即恢复」先变成已恢复；再点已恢复，或已恢复满 15 分钟，会回到质量并开始重新累计窗口。门槛不会被清掉。',
        qualityResume: '立即恢复',
        qualityResumeSuccess: '已立即恢复该用户调度',
        qualityResumeFailed: '恢复该用户调度失败',
        qualityWindowStartSuccess: '已开始重新累计 15 分钟窗口',
        qualityGateChip: '质量',
        qualityBlockedChip: '已停',
        qualityBlockedChipTitle: '当前 15 分钟质量未达标，已停止把此账号调度给该用户',
        qualityResumedChip: '已恢复',
        qualityResumedChipTitle: '已放行该用户。再点一次开始重新累计 15 分钟窗口；满 15 分钟也会自动开始。',
      ...
      stability: {
        ...
        resumeNow: '立即恢复调度',
        resumeHint: '不用等暂停时间结束。若当前 15 分钟窗口仍越界，下次评估（约 5 分钟）可能再次暂停。',
        ...
        useGlobal: '使用系统默认模板',
        useGlobalHint: '关闭后可覆盖本账号的阈值、样本和与/或条件',
        maxP50: '最高 p50 首字（毫秒）',
        ...
        failoverToggleHint:
          '默认关：智能调度门槛和硬关闭继续用现有口径（客户端 status≥400，已排除模型不存在）。打开后，调度才把本号 Recovered 429/503 算进 ErrorCount。开关写入全站质量模板，不是只存在浏览器里。保存后列表数字立即更新；选号热路径最多等下一轮 5 分钟质量 tick。',
