  ...
  admin: {
    ...
    users: {
      ...
      smartSchedule: {
        ...
        windowN: '窗口样本数 N',
        windowNHint: '该用户×该账号最近 N 条完成样本。首字窗和成功窗共用这一个 N，范围 1–100，默认 10。未满 N 不因该指标冷却。',
        probeConcurrency: '考察并发',
        probeConcurrencyHint:
          '考察期进行中上限；跟随 N 时等于窗口 N（不是账号质量 N）。若该账号设了配对 cap，实际占用仍不超过 cap（min(所选, cap)）。',
        probeConcurrencyFollowN: '跟随 N',
        probeConcurrencyCustom: '自定义',
        probeConcurrencyInvalid: '自定义考察并发必须是 1–100 的整数，不能为空',
        pairQuality: '配对质量',
        pairQualityHint: '仅本用户 × 本账号的最近 N 条窗口：首字 p50、成功率、两窗条数/N。点击打开配对趋势与冷却/豁免期/可调度事件。不是账号 last-N 质量。',
        pairQualityClickHint: '打开配对质量详情',
        pairQualityOpenShort: '详情',
        pairQualityDialogTitle: '{name} · 配对质量',
        pairSuccessShort: '成功',
        pairWindowLabel: '窗口条数',
        pairWindowCounts: '首字 {ttft}/{n} · 成功 {ok}/{n}',
