  ...
  admin: {
    ...
    users: {
      ...
      smartSchedule: {
        ...
        maxSlowInWindowHint: '考察期：满首字窗口后超过阈值的条数达到此值即冷却（原 K）。留空且已配首字中位数门槛时默认 2。',
        maxConsecutiveSlow: '连续超标条数',
        maxConsecutiveSlowHint: '考察期：末尾连续此条数均超过阈值即冷却，未满窗也评（原 C）。留空且已配首字中位数门槛时默认 2。',
        schedWindowN: '窗口条数',
        schedWindowNHint: '调度期（可调度）首字/耗时评定窗口长度，只约束中位数：满此条数才评 p50。留空则调度期仍为旧版仅中位数；填写后启用综合门（默认 20/6/3）。',
        schedMaxSlowInWindow: '窗口内超标条数',
        schedMaxSlowInWindowHint: '调度期：满「超标条数」即可评，不必满窗口。超标条数 ≥ 此值即冷却（原 K）。留空不启用综合门。',
        schedMaxConsecutiveSlow: '连续超标条数',
        schedMaxConsecutiveSlowHint: '调度期：满「连续条数」即可评，不必满窗口。末尾连续此条数均超标即冷却（原 C）。留空不启用综合门。',
        ...
        pairQuality: '配对质量',
        pairQualityHint: '仅本用户 × 本账号的最近窗口：首字 p50、成功率、两窗条数各跟自己的 N。点击打开配对趋势与冷却/豁免期/可调度事件。不是账号 last-N 质量。',
