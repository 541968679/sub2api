...
describe('AccountsView oauth fleet usage i18n', () => {
  it('labels used/capacity pool model and fraction display', () => {
    ...
    expect(en.admin.accounts.oauthFleetUsage.burnLine).toContain('{rate}')
    expect(zh.admin.accounts.oauthFleetUsage.burnLine).toContain('{rate}')
    expect(en.admin.accounts.oauthFleetUsage.burnInsufficient).toBeTruthy()
    expect(zh.admin.accounts.oauthFleetUsage.burnInsufficient).toBeTruthy()
  })
})
